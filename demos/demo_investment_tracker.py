...
from modules.finance import GoalType, OptionsLeg, StrategyType, get_investment_integration
...
def run_investment_demo():
    ...
    if trade_response["goal_allocation"]:
        ...
        for i, suggestion in enumerate(allocation["suggestions"][:3], 1):
            status = (
                "COMPLETES GOAL! 🎉"
                if suggestion.get("would_complete")
                else f"→ {suggestion['new_progress']:.1f}%"
            )
            print(
                f"   {i}. {suggestion['goal_name']}: ${suggestion['suggested_amount']:.2f} ({status})"
            )
    ...
    for goal in goals_summary["goals"]:
        progress_bar = "█" * int(goal["progress_percentage"] / 5) + "░" * (
            20 - int(goal["progress_percentage"] / 5)
        )
        print(f"   {goal['name']}:")
        print(f"   [{progress_bar}] {goal['progress_percentage']:.1f}%")
        print(
            f"   ${goal['current_amount']:.2f} / ${goal['target_amount']:,.2f} (${goal['remaining_amount']:,.2f} remaining)"
        )
        print()
