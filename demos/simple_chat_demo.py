...
class SimpleEAIDemo:
    ...
    def _analyze_emotional_continuity(self) -> float:
        ...
        emotional_count = sum(
            1 for msg in recent if msg.get("metadata", {}).get("emotional_override", False)
        )
    ...
    def print_response(self, response, user_input: str):
        ...
        emoji_map = {"emotional": "💖", "balanced": "💭", "objective": "🤔"}
        ...
        print(
            f"📊 {response.tone.title()} tone • {response.confidence:.0%} confidence • {response.resolution_strategy}"
        )
...
async def main():
        ...
        try:
            ...
            if user_input.lower() == "quit":
                ...
                try:
                    ...
                    print(
                        "❌ Invalid strategy. Options: logic_dominant, emotional_priority, harmonic, adaptive"
                    )
                continue
            elif not user_input:
                continue
            ...
            if any(
                ...
                for word in ["sad", "upset", "scared", "worried", "anxious"]
            ):
                context = "emotional_support"
            elif any(
                word in user_input.lower() for word in ["analyze", "logic", "data", "calculate"]
            ):
                context = "analytical_task"
            elif any(
                word in user_input.lower() for word in ["meaning", "purpose", "spiritual", "deep"]
            ):
    ...
    if demo.conversation_history:
        emotional_responses = sum(
            1
            for msg in demo.conversation_history
        ...
        avg_confidence = sum(
            msg.get("metadata", {}).get("confidence", 0) for msg in demo.conversation_history
