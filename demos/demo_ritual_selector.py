...
def generate_comprehensive_ritual_data():
    ...
    for ritual_id, definition in ritual_definitions.items():
        ...
        is_available = random.random() < availability_weights.get(
            definition["activation_method"], 0.6
        )
...
def analyze_ritual_patterns(rituals, symbols, history, offers):
    ...
    print("🕯️ Ritual Availability:")
    print(
        f"   Available: {len(available_rituals)}/{len(rituals)} ({len(available_rituals)/len(rituals)*100:.1f}%)"
    )
    ...
    for symbol in sorted_symbols[:5]:
        print(
            f"   {symbol['name']:12} | {symbol['frequency']:2} invocations (salience: {symbol['salience_score']:.2f})"
        )
    ...
    recent_invocations = [
        h
        for h in history
        if (datetime.now() - datetime.fromisoformat(h["invoked_at"].replace("Z", ""))).days < 7
    ]
