...
class HRMSystemDemo:
    ...
    def __init__(self):
        ...
        print(
            f"   🔒 Identity Layer Protection: {'ENABLED' if self.identity_layer_locked else 'DISABLED'}"
        )

    def update_identity_layer(
        self, identity_data: dict[str, Any], admin_key: Optional[str] = None
    ) -> dict[str, Any]:
        """
        ...
        if self.identity_layer_locked and admin_key != self.identity_admin_key:
            ...
            print("🚨 SECURITY: Unauthorized identity layer access attempt")
        ...
        try:
            ...
            print("✅ ADMIN: Identity layer updated successfully")
            ...
            error_result = {
                "success": False,
                "error": "UPDATE_FAILED",
                "message": f"Identity layer update failed: {e!s}",
                "timestamp": datetime.now().isoformat(),
            }

            print(f"❌ ERROR: Identity layer update failed: {e!s}")
            return error_result
    ...
    def _log_security_violation(self, violation_data: dict[str, Any]):
        ...
        try:
            ...
            print(f"   ⚠️  Failed to log security violation: {e!s}")
    ...
    def test_identity_layer_protection(self):
        ...
        for i, scenario in enumerate(test_scenarios, 1):
            ...
            success_matches_expected = (
                result["success"] and scenario["expected_outcome"] == "ALLOWED"
            ) or (not result["success"] and scenario["expected_outcome"] == "DENIED")

            print(f"   Result: {'✅ PASS' if success_matches_expected else '❌ FAIL'}")
            print(
                f"   Expected: {scenario['expected_outcome']}, Got: {'ALLOWED' if result['success'] else 'DENIED'}"
            )
    ...
    async def _process_scenario(self, scenario_num: int, scenario: dict[str, Any]):
        ...
        try:
            ...
            step3_result = {
                ...
                "personality_applied": formatted_response.metadata.get(
                    "target_personality", "unknown"
                ),
            ...
            if self.demo_config["show_intermediate_steps"]:
                print(
                    f"   🎭 Personality Applied: {formatted_response.metadata.get('target_personality', 'unknown')}"
                )
                print(
                    f"   🔧 Adjustments Made: {', '.join(formatted_response.personality_adjustments)}"
                )
            ...
            if self.demo_config["enable_mirror_mode"]:
                ...
                mirror_reflection = (
                    enhanced_response[len(final_response) :].strip()
                    if enhanced_response != final_response
                    else None
                )
            ...
            print(
                f"🎯 Overall Pipeline Confidence: {self._calculate_pipeline_confidence(scenario_result):.2f}"
            )
            ...
            print(f"❌ ERROR in scenario processing: {e!s}")
            scenario_result.update(
                {
                    "error": str(e),
                    "success": False,
    ...
    async def _show_system_analytics(self):
        ...
        print(
            f"   Success Rate: {hrm_status['metrics']['successful_responses']/hrm_status['metrics']['total_requests']*100:.1f}%"
        )
        ...
        successful_scenarios = sum(
            1 for result in self.demo_results if result.get("success", False)
        )
        total_scenarios = len(self.demo_results)
        avg_processing_time = sum(
            result.get("total_processing_time", 0) for result in self.demo_results
        ) / max(total_scenarios, 1)
        avg_confidence = sum(
            result.get("pipeline_confidence", 0) for result in self.demo_results
        ) / max(total_scenarios, 1)
    ...
    async def interactive_mode(self):
        ...
        while True:
            try:
                ...
                print("─" * 50)
                print(
                    f"⚙️  Mode: {hrm_response.processing_mode.value} | 📊 Confidence: {hrm_response.confidence_score:.2f} | ⏱️  Time: {processing_time:.2f}s"
                )
    ...
    def _show_interactive_help(self):
        ...
        print("   • Type any message to process through HRM")
        print("   • 'status' - Show system analytics")
...
async def run_performance_test():
    ...
    for i, message in enumerate(test_messages, 1):
        ...
        try:
            ...
            print(
                f"   ✅ Processed in {processing_time:.3f}s (confidence: {response.confidence_score:.2f})"
            )
