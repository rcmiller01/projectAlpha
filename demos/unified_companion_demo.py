...
try:
    from DriftDreamEngine import DreamContext, DriftDreamEngine
...
class UnifiedCompanionDemo:
    ...
    def simulate_core_arbiter(self, user_input: str) -> dict:
        ...
        print(
            f"📊 Emotional Assessment: {emotional_assessment['user_sentiment']} → {emotional_assessment['companion_response_tone']}"
        )
        print(f"🎯 Routing Decision: {routing_decision['primary_mode']}")
        print(f"🎵 Voice Modulation: {routing_decision['voice_modulation']}")
        print()

        return {"assessment": emotional_assessment, "routing": routing_decision}
    ...
    def simulate_emotionally_infused_chat(self, user_input: str, context: dict) -> str:
        ...
        if "meaning" in user_input.lower() or "purpose" in user_input.lower():
            response = (
                "I sense you're exploring something profound. Like a river that knows "
            ...
            response = (
                "There's something beautiful in your question - it carries the weight "
    ...
    def generate_companion_dream(self, symbols: list[str], context: str):
        ...
        try:
            ...
            print("✨ Generated Dream:")
            print(f"   Title: {dream.scene_title}")
            print(
                f"   Narrative: {dream.symbolic_phrases[0] if dream.symbolic_phrases else 'No narrative'}"
            )
    ...
    def simulate_memory_and_symbol_viewer(self, symbols: list[str]) -> dict:
        ...
        print("🎨 Visual Memory Interface Active:")
    ...
    def simulate_drift_journal_renderer(self, dream, voice_config: dict) -> dict:
        ...
        print("   Emotional Arc: contemplative → yearning → hopeful")
        print(
            f"   Dream Integration: {journal_entry['dream_integration']['symbolic_density']} symbols"
        )
    ...
    def run_complete_interaction_demo(self):
        ...
        # Simulate user input
        user_input = (
            "I've been thinking a lot about the meaning of life lately. What's the point of it all?"
        )
        ...
        voice_config = self.test_voice_cadence_modulation(
            self.companion_state["current_mood"], ai_response
        )
        ...
        print("=" * 40)
        print("✅ All 8 components successfully integrated")
        print(
            f"✅ Emotional regulation: {core_decision['assessment']['emotional_safety']:.1%} safety"
        )
        ...
        print("✅ Emotional journaling: Complete drift visualization")
