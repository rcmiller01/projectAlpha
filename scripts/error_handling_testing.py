...
class ErrorHandlingTester:
    ...
    async def test_api_error_handling(self):
        ...
        error_scenarios = [
            ...
            # Large payload
            {
                ...
                "endpoint": "/api/memories/store",
                "data": {"content": "x" * 10000, "type": "conversation"},
        ...
        async with aiohttp.ClientSession() as session:
            for scenario in error_scenarios:
                try:
                    ...
                    print(
                        f"  {status} {scenario['name']}: HTTP {status_code} ({response_time:.0f}ms)"
                    )

                except Exception as e:
                    print(f"  ❌ {scenario['name']}: Exception - {e!s}")
                    self.results["error_handling_tests"].append(
                        {
                            "scenario": scenario["name"],
                            "endpoint": scenario["endpoint"],
    ...
    async def test_edge_cases(self):
        ...
        edge_cases = [
            ...
            {
                "name": "Large Numbers",
                "endpoint": "/api/autonomy/learning/update",
                "data": {"progress": 999999999, "score": 1e10},
            },
            # Negative numbers where positive expected
            {
                "name": "Negative Numbers",
                "endpoint": "/api/romantic/intimacy",
                "data": {"intensity": -5, "duration": -100},
            },
            # Array boundary conditions
            {
                "name": "Empty Arrays",
                "endpoint": "/api/creative/collaborative/create",
            ...
            for case in edge_cases:
                try:
                    if case.get("test_type") == "concurrent":
                        ...
                        print(
                            f"  {status} {case['name']}: HTTP {status_code} ({response_time:.0f}ms)"
                        )

                except Exception as e:
                    print(f"  ❌ {case['name']}: Exception - {e!s}")
                    self.results["edge_case_tests"].append(
                        {
                            "case": case["name"],
                            "error": str(e),
            ...
            for test in validation_tests:
                try:
                    ...
                    async with session.post(url, json=test["data"]) as response:
                        ...
                        print(
                            f"  {status} {test['name']}: HTTP {status_code} ({response_time:.0f}ms)"
                        )
            ...
            for test in security_tests:
                try:
                    if test.get("test_type") == "rate_limit":
                        ...
                        if "data" in test:
                            async with session.post(
                                url, json=test["data"], headers=headers
                            ) as response:
                                status_code = response.status
                        else:
        ...
        self.results["security_tests"].append(
            {
                ...
                "security_working": rate_limited
                or requests_sent < 50,  # Either rate limited or stopped sending
                "timestamp": datetime.now().isoformat(),
            }
        )
    ...
    def generate_report(self):
        ...
        if self.results["error_handling_tests"]:
            handled_correctly = [
                t for t in self.results["error_handling_tests"] if t.get("handled_correctly", False)
            ]
            print("\n🛡️ Error Handling:")
            print(
                f"  Scenarios handled correctly: {len(handled_correctly)}/{len(self.results['error_handling_tests'])}"
            )

            # Common error response codes
            status_codes = [
                t.get("status_code")
                for t in self.results["error_handling_tests"]
                if "status_code" in t
            ]
            if status_codes:
        ...
        if self.results["edge_case_tests"]:
            handled_gracefully = [
                t for t in self.results["edge_case_tests"] if t.get("handled_gracefully", False)
            ]
            print("\n🔍 Edge Cases:")
            print(
                f"  Cases handled gracefully: {len(handled_gracefully)}/{len(self.results['edge_case_tests'])}"
            )
        ...
        if self.results["data_validation_tests"]:
            validation_working = [
                t
                for t in self.results["data_validation_tests"]
                if t.get("validation_working", False)
            ]
            print("\n🔒 Data Validation:")
            print(
                f"  Validation tests passed: {len(validation_working)}/{len(self.results['data_validation_tests'])}"
            )

        # Security Summary
        if self.results["security_tests"]:
            security_working = [
                t for t in self.results["security_tests"] if t.get("security_working", False)
            ]
            print("\n🔐 Security:")
            print(
                f"  Security tests passed: {len(security_working)}/{len(self.results['security_tests'])}"
            )

        # Overall Resilience Score
        total_tests = (
            len(self.results["error_handling_tests"])
        ...
        total_passed = (
            len(
                [
                    t
                    for t in self.results["error_handling_tests"]
                    if t.get("handled_correctly", False)
                ]
            )
            + len(
                [t for t in self.results["edge_case_tests"] if t.get("handled_gracefully", False)]
