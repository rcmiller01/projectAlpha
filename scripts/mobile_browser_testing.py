...
class MobileResponsivenessTester:
                ...
                for device in self.mobile_devices:
                    ...
                    for test_page in self.test_pages:
                        ...
                        try:
                            ...
                            self.results["mobile_tests"].append(
                                {
                                    ...
                                    "success": all(
                                        test.get("passed", False) for test in mobile_test_results
                                    ),
                            ...
                            status = (
                                "✅"
                                if all(test.get("passed", False) for test in mobile_test_results)
                                else "⚠️"
                            )
                            print(f"    {status} {test_page['name']}: {load_time:.0f}ms")

                        except Exception as e:
                            print(f"    ❌ {test_page['name']}: {e!s}")
                            self.results["mobile_tests"].append(
                                {
                                    "device": device["name"],
                                    "page": test_page["name"],
    ...
    async def run_mobile_tests(self, page, device):
        ...
        try:
            ...
            if device["isMobile"]:
                ...
                readable_text = (
                    small_text["smallTextCount"] / max(small_text["totalTextElements"], 1) < 0.1
                )
            ...
            tests.append({"name": "Mobile Test Suite", "passed": False, "error": str(e)})
            ...
            for browser_name in self.browsers:
                ...
                for test_page in self.test_pages:
                    ...
                    try:
                        ...
                        status = (
                            "✅"
                            if all(test.get("passed", False) for test in browser_tests)
                            else "⚠️"
                        )
                        print(f"    {status} {test_page['name']}: {load_time:.0f}ms")
    ...
    async def run_browser_compatibility_tests(self, page):
        ...
        try:
            ...
            js_working = await page.evaluate(
            ...
            tests.append({"name": "Browser Compatibility Suite", "passed": False, "error": str(e)})
                    ...
                    self.results["accessibility_tests"].append(
                        {
                            ...
                            "score": sum(
                                1 for test in accessibility_tests if test.get("passed", False)
                            ),
    ...
    async def run_accessibility_tests(self, page):
        ...
        try:
            ...
            tests.append({"name": "Accessibility Test Suite", "passed": False, "error": str(e)})
    ...
    def generate_report(self):
        ...
        if self.results["browser_tests"]:
            ...
            for browser in browsers_tested:
                browser_tests = [
                    t for t in self.results["browser_tests"] if t["browser"] == browser
                ]
                browser_passed = [t for t in browser_tests if t.get("success", False)]
                print(
                    f"  {browser.title()}: {len(browser_passed)}/{len(browser_tests)} pages passed"
                )
        ...
        if self.results["accessibility_tests"]:
            ...
            print(
                f"  Compliance rate: {total_accessibility_score/total_accessibility_tests*100:.1f}%"
            )
