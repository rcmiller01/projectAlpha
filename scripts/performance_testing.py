...
class PerformanceTester:
    def __init__(self, base_url="http://localhost:5000"):
        ...
        self.results = {"api_tests": [], "load_tests": [], "memory_tests": [], "database_tests": []}
    ...
    async def test_api_endpoints(self):
        ...
        endpoints = [
            ...
            {
                "method": "GET",
                "path": "/api/creative/personality/traits",
            ...
            {
                "method": "POST",
                "path": "/api/romantic/intimacy",
                "name": "Intimacy Interaction",
                ...
                try:
                    if endpoint["method"] == "GET":
                        ...
                        async with session.post(
                            f"{self.base_url}{endpoint['path']}", json=data
                        ) as response:
                            status_code = response.status
                            response_time = (time.time() - start_time) * 1000
                    ...
                    print(
                        f"  {status} {endpoint['name']}: {response_time:.2f}ms (HTTP {status_code})"
                    )

                except Exception as e:
                    print(f"  ❌ {endpoint['name']}: Failed - {e!s}")
                    self.results["api_tests"].append(
                        {
                            "endpoint": endpoint["name"],
                            "response_time": None,
    ...
    async def load_test(self, concurrent_users=50, requests_per_user=10):
        ...
        async with aiohttp.ClientSession() as session:
            tasks = [user_simulation(session, user_id) for user_id in range(concurrent_users)]
    ...
    def generate_report(self):
        ...
        if api_successful:
            ...
            print(
                f"  Endpoints meeting <{target_response_time}ms target: {fast_endpoints}/{len(response_times)}"
            )
        ...
        if self.results["load_tests"]:
            ...
            load_response_times = [
                t["response_time"] for t in load_successful if "response_time" in t
            ]

            if load_response_times:
                print("\n🔥 Load Test Performance:")
                print(
                    f"  Success rate: {len(load_successful)/len(self.results['load_tests'])*100:.1f}%"
                )
                print(
                    f"  Average response under load: {statistics.mean(load_response_times):.2f}ms"
                )
        ...
        if db_successful:
            ...
            print(
                f"  Successful queries: {len(db_successful)}/{len(self.results['database_tests'])}"
            )
        ...
        api_success_rate = (
            len(api_successful) / len(self.results["api_tests"]) * 100
            if self.results["api_tests"]
            else 0
        )
        load_success_rate = (
            len(load_successful) / len(self.results["load_tests"]) * 100
            if self.results["load_tests"]
            else 100
        )
...
async def main():
    ...
    try:
        ...
        print(f"\n❌ Performance testing failed: {e!s}")
        return False
