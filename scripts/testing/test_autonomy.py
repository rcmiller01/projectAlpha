...
async def test_scene_generation():
    ...
    from ..story.story_scaffold import Location, Mood, TimeOfDay

    manager = SceneManager()
    scene = await manager.generate_scene(
        time=TimeOfDay.EVENING, location=Location.GARDEN, mood=Mood.REVERENT
    )
...
async def test_emotional_memory():
    ...
    manager = SceneManager()
    reactions = {"Mia": "deeply moved", "Solene": "passionately engaged", "Lyra": "sweetly curious"}

    await manager.update_emotional_memory(
        scene="A tender moment in the garden", reactions=reactions
    )
