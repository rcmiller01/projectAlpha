...
class ReflectionAgent:
    ...
    def __init__(self, memory_graph, config: Optional[dict[str, Any]] = None):
        ...
        self.reflection_depth = self.config.get(
    ...
    async def generate_daily_reflection(self, target_date: date) -> dict[str, Any]:
        ...
        try:
            ...
            events = await self.memory_graph.search_events(
                time_range=(start_time, end_time), limit=200
            )

            if len(events) < self.min_events_for_reflection:
                logger.info(
                    f"📝 Insufficient events ({len(events)}) for reflection on {target_date}"
                )
                return {
            ...
            logger.info(
                f"✅ Generated reflection for {target_date}: {len(reflection['summary'])} characters"
            )
    ...
    def _analyze_emotions(self, events: list[Any]) -> dict[str, Any]:
        ...
        return {
            "dominant_emotions": dict(
                sorted(emotion_freq.items(), key=lambda x: x[1], reverse=True)[:5]
            ),
    ...
    def _analyze_salience_distribution(self, events: list[Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "distribution": {"high": high_salience, "medium": medium_salience, "low": low_salience},
    ...
    def _identify_growth_indicators(self, events: list[Any]) -> list[str]:
        ...
        learning_events = [
            ...
            for e in events
            if any(
                word in e.content.lower() for word in ["learn", "understand", "realize", "insight"]
            )
        ]
        if learning_events:
            growth_indicators.append(
                f"Learning and insight: {len(learning_events)} instances of new understanding"
            )
        ...
        if decision_events:
            ...
            if avg_decision_salience > 0.6:
                growth_indicators.append(
                    f"Thoughtful decision-making: {len(decision_events)} significant decisions"
                )
        ...
        reflection_events = [
            e
            for e in events
            if any(word in e.content.lower() for word in ["think", "reflect", "consider", "ponder"])
        ]
        if reflection_events:
            growth_indicators.append(
                f"Self-reflection: {len(reflection_events)} instances of introspection"
            )
        ...
        goal_events = [
            e
            for e in events
            if any(word in e.content.lower() for word in ["goal", "plan", "achieve", "progress"])
        ]
        if goal_events:
            growth_indicators.append(
                f"Goal-oriented thinking: {len(goal_events)} goal-related thoughts"
            )
    ...
    async def _generate_reflection_content(
        self, analysis: dict[str, Any], target_date: date
    ) -> dict[str, Any]:
    ...
    async def _generate_moderate_reflection(
        self, analysis: dict[str, Any], target_date: date
    ) -> str:
        ...
        if avg_salience > 0.7:
            summary += (
                "Overall, today felt rich with meaningful experiences and important realizations."
            )
        elif avg_salience < 0.3:
            summary += "Today was characterized by gentle, everyday moments that form the quiet foundation of life."
        else:
            summary += (
                "The day struck a balance between significant insights and comfortable routine."
            )
    ...
    def _generate_insights(self, analysis: dict[str, Any]) -> list[str]:
        ...
        if high_pct > 25:
            insights.append(
                f"Today was unusually meaningful - {high_pct:.1f}% of experiences were highly significant"
            )
        ...
        if emotional_analysis.get("emotional_variety", 0) > 7:
            insights.append(
                "Experienced rich emotional complexity today - sign of deep engagement with life"
            )
    ...
    def _generate_patterns(self, analysis: dict[str, Any]) -> list[str]:
        ...
        if theme_analysis.get("theme_diversity", 0) > 4:
            patterns.append(
                "Mind ranging across multiple life domains - indication of broad engagement"
            )
        ...
        if trajectory == "improving":
            ...
            patterns.append(
                "Emotional energy decreased as day progressed - consider energy management"
            )
    ...
    def _generate_future_questions(self, analysis: dict[str, Any]) -> list[str]:
        ...
        if primary_theme == "challenges":
            questions.append(
                "What strategies can I develop to better navigate the challenges I'm facing?"
            )
        elif primary_theme == "learning":
            questions.append(
                "How can I build upon today's insights and continue this learning trajectory?"
            )
    ...
    async def generate_weekly_reflection(self, week_start_date: date) -> dict[str, Any]:
        ...
        try:
            ...
            events = await self.memory_graph.search_events(
                time_range=(start_time, end_time), limit=1000
            )
