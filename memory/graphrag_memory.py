...
class GraphRAGMemory:
    ...
    def __init__(self, memory_file: Optional[str] = None):
        ...
        logger.info(
            f"GraphRAG Memory initialized with {self.graph.number_of_nodes()} nodes, "
        ...
        with self._lock:
            try:
                ...
                if not self.graph.has_node(subject):
                    self.graph.add_node(
                        subject, node_type="concept", created_at=timestamp, access_count=0
                    )

                if not self.graph.has_node(object_node):
                    self.graph.add_node(
                        object_node, node_type="concept", created_at=timestamp, access_count=0
                    )
                ...
                logger.info(
                    f"Added fact: {subject} -[{relation}]-> {object_node} "
    ...
    def query_related(self, node: str, depth: int = 2, min_confidence: float = 0.1) -> QueryResult:
        """
        ...
        with self._lock:
            try:
                ...
                logger.info(
                    f"Query completed: {node} -> {len(related_concepts)} related concepts "
        ...
        with self._lock:
            try:
                ...
                for node in self.graph.nodes():
                    degree_dict[node] = len(list(self.graph.neighbors(node))) + len(
    ...
    def load_memory(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Memory loaded from {self.memory_file} "
...
def example_usage():
    ...
    for concept in result.related_concepts[:5]:  # Show first 5
        print(
            f"  - {concept['concept']} (relation: {concept['relation_type']}, "
