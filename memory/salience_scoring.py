...
class SalienceScorer:
    ...
    def __init__(self, config: dict[str, Any] = None):
        ...
        self.content_type_scores = self.config.get(
            "content_type_scores",
            {
                "decision": 0.9,
                "emotion": 0.8,
        ...
        self.actor_scores = self.config.get(
            "actor_scores", {"user": 0.8, "dolphin": 0.9, "system": 0.6}
        )

        # Emotion intensity mapping
        self.emotion_intensities = self.config.get(
            "emotion_intensities",
            {
    ...
    async def calculate_salience(
        ...
        try:
            ...
            logger.debug(
                f"⚖️ Calculated salience: {salience:.3f} (emotion: {emotion_score:.2f}, content: {content_score:.2f})"
            )
            return salience
    ...
    async def _calculate_emotion_score(self, emotion_analysis: dict[str, Any]) -> float:
        ...
        try:
            ...
            emotion_count_boost = min(
                0.2, len(emotions) * 0.05
            )  # Slight boost for emotional complexity
    ...
    def _calculate_content_score(self, content: str, event_type: str) -> float:
        ...
        try:
            ...
            if "?" in content or any(
                word in content_lower for word in ["why", "how", "what", "when", "where", "who"]
            ):
                base_score += 0.05
    ...
    async def analyze_salience_patterns(self, events: list[Any]) -> dict[str, Any]:
        ...
        try:
            ...
            actor_averages = {
                actor: sum(scores) / len(scores) for actor, scores in actor_salience.items()
            }
    ...
    async def _analyze_temporal_salience(self, events: list[Any]) -> dict[str, Any]:
        ...
        try:
            ...
            numerator = sum(
                (time_values[i] - mean_time) * (salience_scores[i] - mean_salience)
                for i in range(n)
            )
            ...
            return {
                ...
                "salience_variance": round(
                    sum((s - mean_salience) ** 2 for s in salience_scores) / n, 3
                ),
            }
    ...
    async def calibrate_weights(
        self, sample_events: list[Any], target_distribution: dict[str, float] = None
    ):
    ...
    def get_salience_explanation(
        ...
        try:
            ...
            emotion_score = self.emotion_intensities.get(
                emotion_analysis.get("tone", "neutral"), 0.2
            )
            ...
            if final_score >= 0.8:
                ...
                explanation["reasoning"].append(
                    "Medium-high salience: This event is quite important"
                )
            elif final_score >= 0.4:
                explanation["reasoning"].append(
                    "Medium salience: This event has moderate importance"
                )
            else:
