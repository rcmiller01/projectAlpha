...
def create_test_candidates(names: Optional[list[str]] = None) -> list[QuantizationCandidate]:
    ...
    for name in names:
        ...
        candidates.append(
            QuantizationCandidate(name=name, size_gb=size, file_path=f"models/{name}.bin")
        )
...
def main():
    ...
    parser.add_argument(
        "--output", default="emotional_test_results.json", help="Output file for results"
    )
    ...
    if args.analysis_only:
        try:
            with open(args.analysis_only) as f:
                results = json.load(f)
