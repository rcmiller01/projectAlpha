...
def check_drift_rate_limit(session_token: str) -> bool:
    ...
    # Clean old requests
    while (
        drift_requests[session_token] and drift_requests[session_token][0] < current_time - 3600
    ):  # 1 hour window
...
def validate_drift_measurement(measurement_data: dict[str, Any]) -> tuple[bool, str]:
    ...
    try:
        ...
        logger.error(f"Error validating drift measurement: {e!s}")
...
def detect_symbolic_anomaly(measurement: "DriftMeasurement") -> bool:
    ...
    try:
        ...
        if (
            measurement.intimacy_score > extreme_threshold
            or measurement.vulnerability_score > extreme_threshold
            or measurement.trust_score > extreme_threshold
        ):
            logger.warning(
                f"Extreme symbolic drift detected: intimacy={measurement.intimacy_score}"
            )
        ...
        logger.error(f"Error detecting symbolic anomaly: {e!s}")
        return False
...
def log_symbolic_activity(
    ...
    try:
        ...
        logger.error(f"Error logging symbolic activity: {e!s}")
...
class DriftMeasurement:
    ...
    def __post_init__(self):
        ...
        if self.tags:
            self.tags = [
                sanitize_symbolic_input(tag, 50) for tag in self.tags[:MAX_TAGS_PER_MEASUREMENT]
            ]
...
class RitualThreshold:
    ...
    def __post_init__(self):
        ...
        if self.memory_tags:
            self.memory_tags = [
                sanitize_symbolic_input(tag, 50)
                for tag in self.memory_tags[:MAX_TAGS_PER_MEASUREMENT]
            ]
...
class SymbolicDriftManager:
    ...
    def _load_drift_history(self):
        ...
        if self.drift_history_path.exists():
            try:
                ...
                logger.info(
                    f"Loaded {len(self.drift_measurements)} drift measurements and {len(self.ritual_thresholds)} ritual thresholds"
                )
    ...
    def record_drift_measurement(
        ...
        logger.debug(
            f"Recorded drift: intimacy={intimacy:.2f}, vulnerability={vulnerability:.2f}, stability={stability_index:.2f}"
        )
    ...
    def _calculate_stability_index(self) -> float:
        ...
        variance = sum((score - mean_intimacy) ** 2 for score in intimacy_scores) / len(
            intimacy_scores
        )
    ...
    def evaluate_intimacy_unlock_conditions(
        self, memory_manager=None, core_arbiter=None, interaction_context=None
    ) -> dict[str, Any]:
        ...
        if conditions["intimacy_score"] > self.intimacy_unlock_threshold:
            ...
            conditions["reasoning"].append(
                f"Intimacy score {conditions['intimacy_score']:.2f} > {self.intimacy_unlock_threshold}"
            )
        ...
        unlock_conditions_met = (
            conditions["intimacy_threshold_met"]
            and (conditions["ritual_threshold_crossed"] or conditions["shared_vulnerability"])
            and conditions["stability_achieved"]
        )
    ...
    def get_recent_drift_summary(self, hours: int = 24) -> dict[str, Any]:
        ...
        avg_vulnerability = sum(m.vulnerability_score for m in recent_measurements) / len(
            recent_measurements
        )
        avg_trust = sum(m.trust_score for m in recent_measurements) / len(recent_measurements)
        avg_stability = sum(m.stability_index for m in recent_measurements) / len(
            recent_measurements
        )
        ...
        if len(recent_measurements) >= 3:
            early_intimacy = sum(
                m.intimacy_score for m in recent_measurements[: len(recent_measurements) // 2]
            )
            late_intimacy = sum(
                m.intimacy_score for m in recent_measurements[len(recent_measurements) // 2 :]
            )
    ...
    def get_system_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "stability_status": {
                ...
                "stable_for_days": max(
                    [i for i in range(7) if self.is_stable_for_days(i)], default=0
                ),
            },
...
if __name__ == "__main__":
    ...
    async def test_symbolic_drift():
        ...
        for i, scenario in enumerate(test_scenarios):
            ...
            print(
                f"Scenario {i+1}: {scenario['context']} - Intimacy: {scenario['intimacy']:.2f}, Stability: {measurement.stability_index:.2f}"
            )
