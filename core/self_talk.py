...
class SelfTalkSystem:
    ...
    def generate_thought(self) -> Optional[dict]:
        ...
        for pattern in self.thought_patterns:
            if (
                current_state.current_mood in pattern.emotion_triggers
                and relationship.intimacy >= pattern.intimacy_threshold
            ):
                context = self._select_context(pattern.context_triggers)
                thought = pattern.pattern.format(
                    emotion=current_state.current_mood, context=context
                )

                should_share = relationship.trust > 0.5 and current_state.mood_intensity > 0.6
