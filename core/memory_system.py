...
class MemorySystem:
    ...
    def _load_short_term(self) -> dict[str, Any]:
        ...
        if self.short_term_file.exists():
            try:
                with open(self.short_term_file) as f:
                    return json.load(f)
            except Exception as e:
    ...
    def add_message(
        ...
        self.short_term_memory["recent_interactions"] = self.short_term_memory[
            "recent_interactions"
        ][-50:]
    ...
    def _update_emotional_patterns(self, emotion_tags: list[str], sentiment_score: float):
        ...
        for tag in emotion_tags:
            ...
            patterns[tag]["recent_scores"].append(
                {"score": sentiment_score, "timestamp": datetime.now().isoformat()}
            )
    ...
    def add_judgment(
        ...
        session["judgments"].append({"timestamp": datetime.now().isoformat(), **judgment})
    ...
    def get_session_context(
        ...
        recent_sentiments = [
            msg["sentiment_score"] for msg in recent_messages if "sentiment_score" in msg
        ]
        sentiment_trend = (
            sum(recent_sentiments) / len(recent_sentiments) if recent_sentiments else 0.0
        )
    ...
    def get_memory_summary(self) -> dict[str, Any]:
        ...
        total_messages = sum(
            ...
            for session in self.short_term_memory["active_sessions"].values()
        )
        ...
        return {
            ...
            "long_term": {
                ...
                "emotional_patterns_count": len(
                    self.long_term_memory.get("emotional_patterns", {})
                ),
    ...
    def flush_short_term(self) -> bool:
        ...
        try:
            ...
            archive_file = (
                self.memory_dir / f"archive_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
    ...
    def _enforce_quota(self, layer: str):
        ...
        if pruned_count > 0:
            avg_importance_removed = (
                sum(item.get("importance", 0) for item in items_to_remove) / pruned_count
            )
            logger.warning(
