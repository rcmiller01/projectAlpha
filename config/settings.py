...
class ProjectAlphaSettings(BaseModel):
    ...
    DRIFT_SCALING_FACTOR: float = Field(
        default=0.35, 
        description="Scaling factor for emotional drift calculations",
        ge=0.0, 
        le=1.0
    )
    ...
    # === Feature Flags ===
    EMOTION_LOOP_ENABLED: bool = Field(
        default=True, 
        description="Enable emotional processing loop"
    )
    AUTOPILOT_ENABLED: bool = Field(
        default=True, 
        description="Enable autopilot mode"
    )
    SAFE_MODE_FORCE: bool = Field(
        default=False, 
        description="Force safe mode (kill switch)"
    )
    
    # === Security Configuration ===
    ADMIN_MASTER_KEY: Optional[str] = Field(
        default=None, 
        description="Master admin key for sensitive operations"
    )
    ...
    MEMORY_QUOTA_IDENTITY: int = Field(
        default=100, 
        description="Maximum items in identity memory layer",
        ge=10, le=1000
    )
    MEMORY_QUOTA_BELIEFS: int = Field(
        default=500, 
        description="Maximum items in beliefs memory layer",
        ge=50, le=5000
    )
    MEMORY_QUOTA_EPHEMERAL: int = Field(
        default=1000, 
        description="Maximum items in ephemeral memory layer",
        ge=100, le=10000
    )
    ...
    MIRROR_HEALTH_CHECK_ENABLED: bool = Field(
        default=True, 
        description="Enable mirror system health checks"
    )
    ANCHOR_HEALTH_CHECK_ENABLED: bool = Field(
        default=True, 
        description="Enable anchor system health checks"
    )
...
def load_settings() -> ProjectAlphaSettings:
    ...
    config_data = {
        ...
        'MIRROR_HEALTH_CHECK_ENABLED': os.getenv('MIRROR_HEALTH_CHECK_ENABLED', 'true').lower() == 'true',
        'ANCHOR_HEALTH_CHECK_ENABLED': os.getenv('ANCHOR_HEALTH_CHECK_ENABLED', 'true').lower() == 'true',
