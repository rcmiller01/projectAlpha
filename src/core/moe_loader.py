...
class MoELoader:
    ...
    def __init__(self, model_registry: dict[str, ExpertInfo], ram_limit_gb: float = 16.0):
        ...
        logger.info(
            f"MoELoader initialized with {len(self.registry)} experts, {ram_limit_gb}GB RAM limit"
        )
    ...
    def _load_expert(self, key: str) -> Optional[ExpertInstance]:
        ...
        try:
            ...
            instance = ExpertInstance(
                model=model, info=info, loaded_at=time.time(), last_used=time.time()
            )
            ...
            logger.info(
                f"Expert '{key}' loaded successfully. RAM usage: {self.ram_used:.1f}/{self.ram_limit}GB"
            )
            return instance
    ...
    def _ensure_ram_for(self, size_needed: float):
        ...
        while self.ram_used + size_needed > self.ram_limit and self.loaded:
            ...
            lru_key = min(self.loaded.keys(), key=lambda k: self.loaded[k].last_used)
    ...
    def run_experts(
        self, prompt: str, context: Optional[dict] = None, max_experts: int = 3
    ) -> dict[str, Any]:
        """
        ...
        # Select top experts
        selected_experts = sorted(intent_weights.items(), key=lambda x: x[1], reverse=True)[
            :max_experts
        ]
        ...
        for expert_key, weight in selected_experts:
            ...
            if instance:
                future = self.executor.submit(
                    self._run_single_expert, instance, prompt, context, weight
                )
    ...
    def _run_single_expert(
        self, instance: ExpertInstance, prompt: str, context: Optional[dict], weight: float
    ) -> dict[str, Any]:
