...
class EmotionTrainingTracker:
    ...
    def __init__(
        self,
        db_path: str = "emotion_training.db",
        backup_json: str = "emotion_training_backup.json",
    ):
        ...
        logger.info("📊 Emotion Training Tracker initialized")
    ...
    def _init_database(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_model_quant ON training_iterations(model_name, quant_level)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_pass_type ON training_iterations(pass_type, pass_count)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_timestamp ON training_iterations(timestamp)"
            )
    ...
    def _load_backup(self):
        ...
        if self.backup_json.exists():
            try:
                with open(self.backup_json) as f:
        ...
        try:
            ...
            """,
                (
                    ...
                    iteration.quant_level,
                    iteration.pass_type,
                    iteration.pass_count,
                    iteration.iteration,
                    iteration.timestamp,
                    iteration.model_size_mb,
                    iteration.emotional_metrics.response_fluency,
                    iteration.emotional_metrics.emotional_intensity,
                    iteration.emotional_metrics.emotional_match,
                    iteration.emotional_metrics.empathy_score,
                    iteration.emotional_metrics.metaphor_usage,
                    iteration.emotional_metrics.sentiment_accuracy,
                    iteration.emotional_metrics.overall_score(),
                    iteration.notes,
                    iteration.config_hash,
                ),
            )
    ...
    def add_iteration(
        ...
        iteration_num = self.get_next_iteration(
        ...
        logger.info(
            f"➕ Added iteration {iteration_num} for {model_name} ({quant_level.value}, {pass_type.value})"
        )
    ...
    def get_next_iteration(
        self, model_name: str, quant_level: str, pass_type: str, pass_count: int
    ) -> int:
    ...
    def increment_iteration(
        ...
        return self.add_iteration(
            model_name, quant_level, pass_type, pass_count, model_size_mb, emotional_metrics, notes
        )

    def reset_pass(
        self, model_name: str, quant_level: QuantLevel, pass_type: PassType, pass_count: int
    ) -> bool:
        ...
        logger.info(
            f"🗑️ Reset pass: deleted {deleted_count} iterations for {model_name} ({quant_level.value}, {pass_type.value}, pass {pass_count})"
        )
            ...
            comparison["results"].append(
                {
                    ...
                    "efficiency_ratio": round(
                        row["avg_score"] / (row["avg_size"] / 1000), 3
                    ),  # Score per GB
                }
            )
...
def create_cli():
    ...
    add_parser.add_argument(
        "--quant",
        choices=["original", "4bit", "8bit", "gptq", "custom"],
        required=True,
        help="Quantization level",
    )
    add_parser.add_argument(
        "--pass-type", choices=["pass_1", "pass_2"], required=True, help="Training pass type"
    )
    ...
    add_parser.add_argument(
        "--intensity", type=float, required=True, help="Emotional intensity (0-1)"
    )
    ...
    export_parser.add_argument(
        "--format", choices=["csv", "json"], required=True, help="Export format"
    )
    ...
    try:
        if args.command == "add":
            ...
            for performer in stats["top_performers"]:
                print(
                    f"   {performer['model_name']} ({performer['quant_level']}): {performer['best_score']:.3f}"
                )

            print("\n📊 Pass Type Distribution:")
            for pass_stat in stats["pass_statistics"]:
                print(
                    f"   {pass_stat['pass_type']}: {pass_stat['count']} iterations, avg {pass_stat['avg_score']:.3f}"
                )
