...
class MetricsCollector:
    ...
    def __init__(self, analytics_logger=None):
        ...
        self.model_usage_stats = defaultdict(
            lambda: {"requests": 0, "total_time": 0.0, "errors": 0, "last_used": None}
        )
    ...
    def record_request(
        self, session_id: str, model_used: str, response_time: float, success: bool = True
    ):
        ...
        return {
            ...
            "routing": {
                ...
                "local_percentage": round(
                    (
                        self.routing_stats["local_routes"]
                        / max(self.routing_stats["total_decisions"], 1)
                    )
                    * 100,
                    1,
                ),
                "cloud_percentage": round(
                    (
                        self.routing_stats["cloud_routes"]
                        / max(self.routing_stats["total_decisions"], 1)
                    )
                    * 100,
                    1,
    ...
    def get_model_usage_report(self) -> dict[str, Any]:
        ...
        for model, stats in self.model_usage_stats.items():
            avg_response_time = (
                stats["total_time"] / max(stats["requests"], 1) if stats["requests"] > 0 else 0.0
            )

            error_rate = (
                stats["errors"] / max(stats["requests"], 1) * 100 if stats["requests"] > 0 else 0.0
            )
    ...
    def aggregate_hourly_data(self):
        ...
        hour_metrics = [
            m for m in self.metrics_history if m.timestamp >= current_hour - timedelta(hours=1)
        ]
        ...
        hourly_summary = {
            ...
            "total_requests": max(m.total_requests for m in hour_metrics)
            - min(m.total_requests for m in hour_metrics),
    ...
    def get_health_check(self) -> dict[str, Any]:
        ...
        if issues and health_status == "warning":
            if (
                len(issues) > 2
                or current_metrics.cpu_percent > 95
                or current_metrics.memory_percent > 95
            ):
