...
def check_report_rate_limit(session_token: str) -> bool:
    ...
    # Clean old requests
    while (
        report_requests[session_token] and report_requests[session_token][0] < current_time - 3600
    ):  # 1 hour window
...
def validate_text_input(text: str, max_length: int = MAX_REPORT_TEXT_LENGTH) -> tuple[bool, str]:
    ...
    try:
        ...
        logger.error(f"Error validating text input: {e!s}")
...
def detect_report_anomaly(report: "SelfReport") -> bool:
    ...
    try:
        ...
        if (
            abs(report.emotion.valence) > 0.95
            or report.emotion.arousal > 0.95
            or report.emotion.arousal < 0.05
        ):
            logger.warning(
                f"Extreme emotion values: valence={report.emotion.valence}, arousal={report.emotion.arousal}"
            )
            anomaly_detected = True
        ...
        logger.error(f"Error detecting report anomaly: {e!s}")
        return False
...
def log_report_activity(
    ...
    try:
        ...
        logger.error(f"Error logging report activity: {e!s}")
