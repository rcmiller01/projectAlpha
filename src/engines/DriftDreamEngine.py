...
class DriftDreamEngine:
    ...
    def load_dream_journal(self):
        ...
        try:
            with open(self.dream_journal_file, encoding="utf-8") as f:
                data = json.load(f)
    ...
    def save_dream_journal(self):
        ...
        try:
            ...
            recent_dreams = (
                self.dream_journal[-self.max_dream_history :]
                if len(self.dream_journal) > self.max_dream_history
                else self.dream_journal
            )
    ...
    def echo_symbol(self, symbol: str, dream_mood: str) -> list[str]:
        ...
        """
        base_phrases = self.symbol_dream_phrases.get(
            symbol,
            [
    ...
    def blend_dreams_over_time(self, days_back: int = 7) -> dict[str, Any]:
        ...
        recurring_symbols = [
            ...
            if count >= len(recent_dreams) * 0.3
        ]
        ...
        return {
            ...
            "dream_intensity_avg": sum(d.emotional_intensity for d in recent_dreams)
            / len(recent_dreams),
            "lucidity_trend": sum(d.lucidity_level for d in recent_dreams) / len(recent_dreams),
            "most_common_metaphors": sorted(
                metaphor_themes.items(), key=lambda x: x[1], reverse=True
            )[:5],
        }
    ...
    def get_recent_dreams(self, count: int = 5) -> list[DreamEntry]:
        ...
        return (
            self.dream_journal[-count:] if len(self.dream_journal) >= count else self.dream_journal
        )

    def get_dreams_by_mood(self, mood: str, limit: int = 10) -> list[DreamEntry]:
        """Get dreams that contain a specific mood"""
        matching_dreams = [dream for dream in self.dream_journal if mood in dream.mood_palette]
    ...
    def get_dreams_by_symbol(self, symbol: str, limit: int = 10) -> list[DreamEntry]:
        ...
        matching_dreams.sort(
            key=lambda d: datetime.fromisoformat(d.dream_timestamp.replace("Z", "")), reverse=True
        )
    ...
    def _generate_dream_narrative(
        self, mood: str, resolution: str, symbols: list[str], drift_context: DreamContext
    ) -> list[str]:
        """Generate the main dream narrative phrases"""
        narrative_phrases = []

        # Start with a base template for the mood/resolution
        base_templates = self.dream_templates.get(mood, {}).get(
            resolution, ["I found myself in a place between waking and sleeping"]
        )
    ...
    def _generate_echoed_phrase(
        self, symbolic_phrases: list[str], mood: str, resolution: str
    ) -> str:
    ...
    def _calculate_dream_intensity(self, drift_context: DreamContext) -> float:
        ...
        if drift_context.recent_drift:
            avg_drift = sum(abs(d.get("drift_delta", 0)) for d in drift_context.recent_drift) / len(
    ...
    def _identify_source_drift(self, drift_context: DreamContext) -> Optional[str]:
        ...
        significant_drift = max(
            drift_context.recent_drift, key=lambda d: abs(d.get("drift_delta", 0))
        )
        ...
        if dream_entry.symbol_sources:  # Check if not None
            for symbol in dream_entry.symbol_sources:
                self.symbol_memory.record_symbol_use(
                    symbol, dream_context, co_occurring_symbols=dream_entry.symbol_sources
                )
    ...
    def _calculate_dream_coherence(self, dreams: list[DreamEntry]) -> float:
        ...
        for i in range(len(dreams) - 1):
            ...
            mood_coherence = mood_overlap / max(
                len(dream1.mood_palette), len(dream2.mood_palette), 1
            )
            ...
            if dream1.resolution_state == dream2.resolution_state:
                ...
                dream1.resolution_state == "unresolved"
                and dream2.resolution_state == "transforming"
            ) or (
...
if __name__ == "__main__":
    ...
    test_context = DreamContext(
        ...
        active_symbols={"mirror": 0.8, "thread": 0.6, "river": 0.5, "flame": 0.3},
        mood_trace=[
    ...
    for i in range(3):
        ...
        print(
            f"    Intensity: {dream.emotional_intensity:.2f}, Lucidity: {dream.lucidity_level:.2f}"
        )
