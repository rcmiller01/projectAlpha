...
async def startup_event():
    ...
    try:
        ...
        system_config = {
            ...
            "safety": {"crisis_detection": True, "content_filtering": True},
        }
...
async def process_interaction(
    ...
    try:
        ...
        response_data = await companion.process_interaction(
            user_id=request.user_id, user_input=request.user_input, session_context=session_context
        )

        # Schedule background tasks for data persistence
        background_tasks.add_task(save_interaction_data, interaction_id, request, response_data, db)
        ...
        raise HTTPException(status_code=500, detail=f"Failed to process interaction: {e!s}")
...
async def create_user_profile(
    user_id: str, profile_data: UserProfileRequest, db: DatabaseInterface = Depends(get_database)
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to process user profile: {e!s}")


@app.get("/api/v1/users/{user_id}/profile", response_model=UserProfileResponse)
async def get_user_profile(user_id: str, db: DatabaseInterface = Depends(get_database)):
    """Get user profile"""
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get user profile: {e!s}")


@app.get("/api/v1/users/{user_id}/summary")
async def get_user_summary(
    user_id: str, companion: UnifiedCompanion = Depends(get_companion_system)
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get user summary: {e!s}")


@app.get("/api/v1/users/{user_id}/interactions")
async def get_user_interactions(
    user_id: str, limit: int = 20, db: DatabaseInterface = Depends(get_database)
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get user interactions: {e!s}")


@app.get("/api/v1/sessions/{session_id}", response_model=SessionSummaryResponse)
async def get_session_summary(session_id: str, db: DatabaseInterface = Depends(get_database)):
    """Get session summary"""
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get session summary: {e!s}")
