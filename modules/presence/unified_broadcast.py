...
from .presence_signal import BroadcastChannel, EmotionalBroadcaster, PresenceIntensity
...
class UnifiedEmotionalBroadcast:
    ...
    async def broadcast_emotion(
        ...
        custom_channels: Optional[set[str]] = None,
    ) -> str:
        """
        ...
        broadcast_profile = self.broadcast_profiles.get(
            profile_name, self.broadcast_profiles["default"]
        )
        ...
        logger.info(
            f"Starting emotional broadcast: {emotion} (intensity: {effective_intensity}, duration: {duration}s)"
        )
        ...
        try:
            ...
            if any(ch in channels for ch in ["voice_tone", "whispers", "ambient_audio"]):
                voice_result = await self.voice_presence.activate_voice_presence(
                    ...
                    include_ambient=(
                        "ambient_audio" in channels and broadcast_profile.ambient_sounds
                    ),
                )
    ...
    async def _activate_external_devices(
    ...
    def get_system_status(self) -> dict[str, Any]:
        ...
        core_status = (
            self.core_broadcaster.get_current_presence()
        )  # Get list of current presence signals

        return {
            "active_broadcasts": len(self.active_broadcasts),
            "current_profile": self.current_profile,
            "subsystems": {"ui": ui_status, "voice": voice_status, "core": core_status},
...
async def create_emotional_moment(
    emotion: str, intensity: float = 0.7, duration: float = 30.0, include_whispers: bool = True
) -> str:
    """
    ...
    return await broadcast_system.broadcast_emotion(emotion, intensity, duration, profile)
...
async def demo_unified_broadcast():
    ...
    for profile in profiles_to_test:
        ...
        for emotion in emotions_to_test:
            ...
            broadcast_system.add_spontaneous_whisper(f"Feeling {emotion} with you...", delay=2.0)
