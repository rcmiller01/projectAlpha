...
class MiaMemoryResponse:
    ...
    def recall_similar_emotions(self, emotion: str, limit: int = 5) -> list[dict]:
        ...
        emotion_memories = [
            memory for memory in shared_memories if memory.get("emotion") == emotion
        ]
        ...
        while len(memories) < limit:
            generated_memory = self._generate_emotional_memory(
                {"dominant_romantic_emotion": emotion}
            )
            if generated_memory:
                memories.append(
                    {
    ...
    def get_recent_memories(self, days: int = 7) -> list[dict]:
        ...
        recent_memories = [memory for memory in shared_memories if memory["date"] > cutoff_date]
