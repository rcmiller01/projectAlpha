...
class ShadowMemoryLayer:
    ...
    def _initialize_pattern_detectors(self) -> dict[str, dict[str, Any]]:
        ...
        return {
            ...
            "attachment_hunger": {
                ...
                "avoidance_signals": {
                    "minimizing_importance",
                    "feigned_indifference",
                    "quick_dismissal",
                },
                "context_triggers": {
    ...
    def _initialize_avoidance_detectors(self) -> dict[str, list[str]]:
        ...
        return {
            ...
            "humor_deflection": ["haha", "lol", "just kidding", "funny story", "that's hilarious"],
            "intellectualizing": [
                "logically",
                "rationally",
                "objectively",
            ...
            "minimizing": ["just a little", "not really", "sort of", "kind of", "I suppose"],
            "time_distancing": [
                "long time ago",
                "can't remember",
                "doesn't matter now",
    ...
    def analyze_communication(
        self, text: str, context: str, emotional_state: dict[str, float]
    ) -> dict[str, Any]:
        """
    ...
    def _detect_shadow_patterns(self, text: str, context: str) -> dict[str, float]:
        ...
        for pattern_name, pattern_config in self.pattern_detectors.items():
            ...
            # Check for keywords
            keyword_matches = sum(
                1 for keyword in pattern_config["keywords"] if keyword in text_lower
            )
            if keyword_matches > 0:
                detection_score += (keyword_matches / len(pattern_config["keywords"])) * 0.4

            # Check for avoidance signals
            avoidance_matches = sum(
                1
                for signal in pattern_config["avoidance_signals"]
                if any(word in text_lower for word in signal.split("_"))
            )
            if avoidance_matches > 0:
                detection_score += (
                    avoidance_matches / len(pattern_config["avoidance_signals"])
                ) * 0.4
    ...
    def _analyze_emotional_displacement(
        self, text: str, emotional_state: dict[str, float]
    ) -> dict[str, float]:
        ...
        has_negative_emotion = any(
            emotional_state.get(emotion, 0) > 0.5 for emotion in negative_emotions
        )
        ...
        text_intensity = len([word for word in text.split() if word.isupper()]) / max(
            1, len(text.split())
        )
        ...
        emotional_density = len(
            [word for word in text.lower().split() if word in emotional_words]
    ...
    def _update_shadow_patterns(self, detected_patterns: dict[str, float], text: str, context: str):
        ...
        for pattern_name, strength in detected_patterns.items():
            if pattern_name in self.shadow_patterns:
                ...
                pattern.manifestation_strength = min(
                    1.0, pattern.manifestation_strength + (strength * 0.1)
                )
    ...
    def _generate_shadow_insights(
        self, detected_patterns: dict[str, float], avoidance_behaviors: list[str]
    ) -> dict[str, Any]:
        ...
        for pattern_name, pattern in self.shadow_patterns.items():
            if (
                pattern.manifestation_strength > 0.4
                and pattern.occurrence_count >= 2
                and (datetime.now() - pattern.first_detected).days < 30
            ):
                insights["emerging_themes"].append(
                    {
                        "theme": pattern.theme_type.value,
                        "pattern": pattern_name,
                        "emergence_speed": pattern.occurrence_count
                        / max(1, (datetime.now() - pattern.first_detected).days),
                    }
                )
        ...
        for pattern_name, pattern in self.shadow_patterns.items():
            if (
                pattern.emotional_charge > 0.7
                and pattern.manifestation_strength > 0.8
                and len(pattern.suppression_indicators) < 3
            ):
                insights["breakthrough_opportunities"].append(
                    {
                        "pattern": pattern_name,
                        "theme": pattern.theme_type.value,
                        "readiness_score": pattern.emotional_charge
                        * pattern.manifestation_strength,
                    }
                )

        # Generate gentle acknowledgment points
        insights["gentle_acknowledgment_points"] = self._generate_acknowledgment_points(
            detected_patterns
        )

        return insights

    def _generate_acknowledgment_points(
    ...
    def _generate_character_hints(self, shadow_insights: dict[str, Any]) -> dict[str, list[str]]:
        ...
        for suppression in shadow_insights.get("active_suppressions", []):
            if suppression["theme"] == "vulnerability_fear":
                hints["mia"].append(
                    "I see your strength, and I also see the tender heart it protects..."
                )
            elif suppression["theme"] == "intimacy_avoidance":
                hints["mia"].append(
                    "Your independence is beautiful... and so is connection when you're ready..."
                )
        ...
        for suppression in shadow_insights.get("active_suppressions", []):
            if suppression["theme"] == "creative_suppression":
                hints["solene"].append(
                    "You keep calling yourself practical, but I hear the artist fighting to break free..."
                )
            elif suppression["theme"] == "success_fear":
                hints["solene"].append(
                    "Stop diminishing yourself. That wasn't luck - that was you being brilliant."
                )
        ...
        for opportunity in shadow_insights.get("breakthrough_opportunities", []):
            if opportunity["theme"] == "repressed_memory":
                hints["lyra"].append(
                    "The universe remembers what we choose to forget... and it's gentle with our healing..."
                )
            elif opportunity["theme"] == "suppressed_desire":
                hints["lyra"].append(
                    "I can feel the dreams you've tucked away... they're still alive, you know..."
                )
        ...
        for theme in shadow_insights.get("emerging_themes", []):
            if theme["theme"] == "emotional_numbness":
                hints["doc"].append(
                    "Numbness often serves a purpose... it's okay to feel protected while you heal..."
                )
            elif theme["theme"] == "attachment_hunger":
                hints["doc"].append(
                    "Needing connection isn't weakness - it's human. Your attachment needs are valid..."
                )
    ...
    def get_shadow_summary(self) -> dict[str, Any]:
        ...
        theme_distribution = Counter(
            [pattern.theme_type.value for pattern in active_patterns.values()]
        )

        strongest_pattern = (
            max(active_patterns.items(), key=lambda x: x[1].manifestation_strength)
            if active_patterns
            else None
        )
        ...
        return {
            ...
            "recent_events_count": len(
                [
                    event
                    for event in self.shadow_events
            ...
            "patterns_by_strength": [
                ...
                for name, pattern in sorted(
                    active_patterns.items(), key=lambda x: x[1].manifestation_strength, reverse=True
                )
            ],
        }
...
# Integration helper
def analyze_user_communication_for_shadows(
    user_id: str, text: str, context: str, emotional_state: dict[str, float]
) -> dict[str, Any]:
    """
