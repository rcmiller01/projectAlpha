...
def store_scene_memory(
    symbol: str, mood: str, persona: str, clip_id: str, trigger_phrase: str = ""
):
    scene = {
...
def _append_scene(scene):
    ...
    if os.path.exists(SCENE_LOG_PATH):
        with open(SCENE_LOG_PATH, encoding="utf-8") as f:
            scenes = json.load(f)
    scenes.append(scene)
...
def recall_scene(symbol=None, mood=None, persona=None):
    ...
    for scene in reversed(scenes):
        if (
            (not symbol or scene["symbol"] == symbol)
            and (not mood or scene["mood"] == mood)
            and (not persona or scene["persona"] == persona)
        ):
            return scene
...
def auto_capture_scene_if_sacred(mood: str, symbol: str):
    ...
    if mood in SACRED_MOODS:
        clip_id = (
            f"{persona.lower()}_{symbol}_{mood}_{datetime.now().strftime('%Y%m%d%H%M%S')}.webm"
        )
        store_scene_memory(
            symbol=symbol,
            mood=mood,
