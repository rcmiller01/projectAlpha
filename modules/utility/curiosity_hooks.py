...
class CuriosityHooks:
    ...
    def update_interest_from_journal(self, journal_entries: list[dict[str, Any]]) -> bool:
        ...
        for entry in recent_entries:
            ...
            interesting_words = [
                word for word in content.split() if len(word) > 5 and word.isalpha()
            ]
    ...
    async def discover_content(self, max_items: int = 3) -> list[CuriosityItem]:
        ...
        for content in mock_content:
            ...
            if relevance_score >= 0.3:
                item_id = hashlib.md5(f"{content['title']}{content['url']}".encode()).hexdigest()[
                    :12
                ]
    ...
    def get_unread_discoveries(self, limit: int = 5) -> list[CuriosityItem]:
        ...
        unread = [item for item in self.discoveries if item.read_status == "unread"]

        # Sort by relevance score and recency
        unread.sort(key=lambda x: (x.relevance_score, x.discovery_time), reverse=True)
