...
class ThreadManager:
    ...
    def _load_folders(self) -> dict[str, dict]:
        ...
        try:
            with open(self.folders_file) as f:
                return json.load(f)
        except Exception as e:
    ...
    def create_thread(self, request: CreateThreadRequest) -> Thread:
        """Create a new thread"""
        thread = Thread(
            title=request.title, folder_id=request.folder_id, folder_name=request.folder_name
        )

        # Add initial message if provided
        if request.initial_message:
            initial_msg = ThreadMessage(role="user", content=request.initial_message)
    ...
    def list_threads(
        self, folder_id: Optional[str] = None, include_archived: bool = False
    ) -> list[Thread]:
...
def setup_thread_routes(app: FastAPI):
    ...
    async def list_threads(folder_id: Optional[str] = None, include_archived: bool = False):
        """List threads, optionally filtered by folder"""
        try:
