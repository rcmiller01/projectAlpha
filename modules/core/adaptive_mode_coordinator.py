...
class AdaptiveModeCoordinator:
    ...
    async def process_interaction(self, user_input: str, context: dict) -> GuidancePackage:
        ...
        enhanced_guidance = await self._apply_mode_configuration(
            base_guidance, detected_mode, context
        )

        # 5. Handle mode transitions if needed
        if previous_mode != detected_mode:
            await self._handle_mode_transition(
                enhanced_guidance, previous_mode, detected_mode, context
            )
    ...
    async def _detect_optimal_mode(self, user_input: str, context: dict) -> str:
        ...
        context_analysis = await self.context_detector.analyze_interaction(
    ...
    async def _apply_user_preferences(
        self, detected_mode: str, analysis: dict, context: dict
    ) -> str:
    ...
    async def _apply_mode_configuration(
        self, base_guidance: GuidancePackage, mode: str, context: dict
    ) -> GuidancePackage:
    ...
    async def _enhance_personal_guidance(
        self, guidance: GuidancePackage, context: dict
    ) -> GuidancePackage:
        ...
        guidance.attachment_guidance += (
            "Use warm, caring language and create space for emotional vulnerability."
        )
        ...
        guidance.scene_guidance = (
            "Create a warm, intimate environment with soft lighting and comfortable atmosphere. "
        )
        guidance.scene_guidance += (
            "The space should feel safe, private, and conducive to emotional sharing."
        )

        return guidance

    async def _enhance_development_guidance(
        self, guidance: GuidancePackage, context: dict
    ) -> GuidancePackage:
        ...
        guidance.therapeutic_guidance += (
            "Provide technical guidance with emotional encouragement and stress management."
        )

        # Scene guidance for productivity
        guidance.scene_guidance = (
            "Create a focused, organized workspace with good lighting and minimal distractions. "
        )
        guidance.scene_guidance += (
            "The environment should feel productive yet comfortable and supportive."
        )

        return guidance

    async def _enhance_creative_guidance(
        self, guidance: GuidancePackage, context: dict
    ) -> GuidancePackage:
        ...
        guidance.creative_guidance += (
            "Collaborate actively in creative process and help overcome blocks."
        )
    ...
    async def _enhance_crisis_guidance(
        self, guidance: GuidancePackage, context: dict
    ) -> GuidancePackage:
        ...
        guidance.therapeutic_guidance = (
            "CRISIS INTERVENTION MODE: Provide immediate emotional support and safety assessment. "
        )
        guidance.therapeutic_guidance += (
            "Focus on presence, validation, and connection. Do not attempt problem-solving. "
        )
        ...
        guidance.scene_guidance += (
            "Use gentle lighting and eliminate any elements that might feel overwhelming."
        )
    ...
    async def _enhance_hybrid_guidance(
        self, guidance: GuidancePackage, context: dict
    ) -> GuidancePackage:
        ...
        guidance.attachment_guidance += (
            "Maintain emotional connection while addressing all user needs comprehensively."
        )
        ...
        # Balanced scene
        guidance.scene_guidance = (
            "Create a balanced environment that supports both focused work and emotional comfort. "
        )
        guidance.scene_guidance += (
            "The space should feel adaptable to different needs while maintaining consistency."
        )

        return guidance

    async def _handle_mode_transition(
        self, guidance: GuidancePackage, previous_mode: str, new_mode: str, context: dict
    ):
        ...
        if previous_mode == "personal" and new_mode == "development":
            ...
            transition_guidance += (
                "Immediately prioritize safety and emotional support, setting aside other concerns."
            )
        elif previous_mode == "crisis":
            transition_guidance += (
                "Gently transition back to normal support while monitoring for stability."
            )
        else:
            transition_guidance += (
                "Smoothly adapt to new needs while maintaining relationship continuity."
            )
    ...
    def _update_user_preferences(self, mode: str, context: dict):
        ...
        total_interactions = sum(
            prefs.get("usage_count", 0)
            for prefs in self.user_preferences["mode_preferences"].values()
        )
