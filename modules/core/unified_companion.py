...
from .crisis_safety_override import CrisisLevel, CrisisSafetyOverride
from .enhanced_logging import DecisionCategory, EnhancedLogger
...
class EmotionalWeightTracker:
    ...
    def _update_attachment_indicators(
        self, user_id: str, context: dict[str, Any], emotional_state: dict[str, float]
    ):
        ...
        if emotional_state.get("anxiety", 0) > 0.7 and context.get("seeking_reassurance", False):
            attachment_indicators["anxious_attachment"] = (
                attachment_indicators.get("anxious_attachment", 0) + 0.1
            )

        # Avoidance indicators
        if emotional_state.get("detachment", 0) > 0.6:
            attachment_indicators["avoidant_attachment"] = (
                attachment_indicators.get("avoidant_attachment", 0) + 0.1
            )

        # Secure attachment indicators
        if emotional_state.get("trust", 0) > 0.8 and emotional_state.get("openness", 0) > 0.7:
            attachment_indicators["secure_attachment"] = (
                attachment_indicators.get("secure_attachment", 0) + 0.1
            )
    ...
    def _calculate_stability(
        self, weights: dict[str, float], history: list[dict[str, Any]]
    ) -> float:
...
class DynamicTemplateEngine:
    ...
    async def select_optimal_template(
        self, user_id: str, context: dict[str, Any], available_templates: dict[str, list[str]]
    ) -> dict[str, str]:
        ...
        for category, templates in available_templates.items():
            ...
            for i, template in enumerate(templates):
                ...
                # Combined score
                template_scores[i] = (
                    effectiveness_score * 0.5 + variety_score * 0.3 + context_score * 0.2
                )
    ...
    def _calculate_context_relevance(self, template: str, context: dict[str, Any]) -> float:
        ...
        for emotion, intensity in emotional_state.items():
            if intensity > 0.5 and emotion in relevance_mapping:
                ...
                keyword_matches = sum(
                    1 for keyword in relevant_keywords if keyword in template_lower
                )
    ...
    async def update_template_effectiveness(
        self, user_id: str, category: str, template_index: int, effectiveness_rating: float
    ):
...
class SymbolicContextManager:
        ...
        if symbolic_elements:
            symbolic_memory = {
                ...
                "significance_score": self._calculate_significance(
                    symbolic_elements, interaction_data
                ),
            }
    ...
    def _calculate_significance(
        self, symbolic_elements: dict[str, Any], interaction_data: dict[str, Any]
    ) -> float:
        """Calculate significance score for symbolic memory"""
        base_significance = (
            sum(symbolic_elements.values()) / len(symbolic_elements) if symbolic_elements else 0
        )
    ...
    def get_relevant_symbolic_context(
        self, user_id: str, current_themes: list[str]
    ) -> dict[str, Any]:
        ...
        for memory in memories:
            ...
            if theme_overlap > 0:
                relevant_memories.append(
                    {**memory, "relevance_score": theme_overlap / len(memory_themes)}
                )

        # Sort by relevance and significance
        relevant_memories.sort(
            key=lambda x: x["relevance_score"] * x["significance_score"], reverse=True
        )
...
class UnifiedCompanion:
    ...
    async def _initialize_memory_system(self):
        ...
        if self.database:
            try:
                ...
                for mem in memories:
                    self.memory_system["long_term"].setdefault(mem.memory_type, []).append(
                        mem.content
                    )
            except Exception as e:
    ...
    async def process_interaction(
        self, user_id: str, user_input: str, session_context: Optional[dict[str, Any]] = None
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            if await self.crisis_override.check_interrupt_required(
            ...
            if crisis_assessment.immediate_response_needed:
                ...
                (
                    ...
                    crisis_response,
                    intervention,
                ) = await self.crisis_override.trigger_crisis_override(
            ...
            relevant_memories = await self._retrieve_relevant_memories(
                user_id, user_input, interaction_state
            )
            ...
            self.enhanced_logger.log_decision(
                ...
                "Detecting optimal interaction mode",
                {
                    ...
                    "emotional_context": analysis_context.get("current_emotional_state", {}),
                },
            ...
            response_quality = await self._estimate_response_quality(
                companion_response, analysis_context
            )
            for category, template in optimal_templates.items():
                template_index = (
                    available_templates[category].index(template)
                    if template in available_templates[category]
                    else 0
                )
            ...
            await self._update_memory_and_state(
                interaction_state, user_input, companion_response, analysis_context
            )

            # STEP 13: Handle Utility Actions
            if guidance_package.utility_actions:
                await self._handle_utility_actions(
                    guidance_package.utility_actions, interaction_state
                )
            ...
            enhanced_context_analysis = {
                ...
                "emotional_patterns": self.emotional_weight_tracker.get_emotional_pattern_analysis(
                    user_id
                ),
            ...
            if session_context and session_context.get("end_session"):
                ...
                response["companion_response"] += "\n\n" + self.goodbye_protocol.select_goodbye(
                    style
                )
            ...
            self.enhanced_logger.finish_interaction_trace({"type": "error", "error": str(e)})

            # Enhanced fallback response
            return await self._generate_enhanced_fallback_response(user_input, str(e), user_id)

    async def _get_interaction_state(
        self, user_id: str, session_context: Optional[dict[str, Any]]
    ) -> InteractionState:
    ...
    def _build_context_for_analysis(self, interaction_state: InteractionState) -> dict[str, Any]:
        ...
        return {
            ...
            "interaction_count": interaction_state.interaction_count,
            "session_duration": (
                datetime.now() - interaction_state.last_interaction
            ).total_seconds(),
            "adaptive_profile": interaction_state.adaptive_profile,
        }

    async def _update_interaction_state(
        self, interaction_state: InteractionState, context_analysis: dict[str, Any], user_input: str
    ):
    ...
    async def _generate_module_guidance(
        self, context_analysis: dict[str, Any], interaction_state: InteractionState
    ) -> dict[str, Any]:
    ...
    async def _generate_companion_response(
        ...
        if isinstance(guidance, dict) and "mode_specifics" in guidance:
            ...
            prompt = await self._build_enhanced_prompt(
                user_input, context_analysis, guidance, interaction_state
            )
        else:
...
- Interaction Count: {interaction_state.interaction_count}"""
    ...
    def _build_guidance_context(
        self, guidance: dict[str, Any], context_analysis: dict[str, Any]
    ) -> str:
    ...
    def _select_response_patterns(
        self, primary_focus: str, context_analysis: dict[str, Any]
    ) -> str:
    ...
    async def _update_memory_and_state(
        self,
        interaction_state: InteractionState,
        user_input: str,
    ...
    async def _update_adaptive_profile(
        self, interaction_state: InteractionState, context_analysis: dict[str, Any]
    ):
        ...
        if context_analysis.get("emotional_priority", "medium") == "high":
            profile["emotional_support_frequency"] = (
                profile.get("emotional_support_frequency", 0) + 1
            )

        # Track technical assistance patterns
        if context_analysis.get("technical_priority", "medium") == "high":
            profile["technical_assistance_frequency"] = (
                profile.get("technical_assistance_frequency", 0) + 1
            )

        # Track creative collaboration patterns
        if context_analysis.get("creative_priority", "medium") == "high":
            profile["creative_collaboration_frequency"] = (
                profile.get("creative_collaboration_frequency", 0) + 1
            )
    ...
    async def _generate_fallback_response(self, user_input: str, error_msg: str) -> dict[str, Any]:
        ...
        return {
            ...
            "interaction_metadata": {"fallback_response": True, "error_logged": True},
        }
    ...
    async def get_interaction_summary(self, user_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "adaptive_profile": interaction_state.adaptive_profile,
            "recent_conversation_themes": self._analyze_recent_themes(
                interaction_state.conversation_history
            ),
        }
    ...
    def _analyze_recent_themes(self, conversation_history: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        primary_focuses = [
            interaction.get("context_analysis", {}).get("primary_focus", "general")
            for interaction in recent_interactions
        ]

        emotional_priorities = [
            interaction.get("context_analysis", {}).get("emotional_priority", "medium")
            for interaction in recent_interactions
        ]

        return {
            "primary_focus_distribution": {
                focus: primary_focuses.count(focus) for focus in set(primary_focuses)
            },
            "emotional_priority_distribution": {
                priority: emotional_priorities.count(priority)
    ...
    async def _handle_utility_actions(
        self, utility_actions: list[dict], interaction_state: InteractionState
    ):
        ...
        for action in utility_actions:
            ...
            if action_type == "crisis_logging":
                self.logger.warning(
                    f"Crisis level {action.get('level', 0)} detected for user {interaction_state.user_id}"
                )
                ...
                interaction_state.adaptive_profile["preferences"].update(
    ...
    async def _log_crisis_intervention(self, user_id: str, intervention):
        ...
        try:
            ...
            if hasattr(self.database, "log_crisis_event"):
                ...
                self.enhanced_logger.critical(
                    f"Crisis intervention for user {user_id}: {crisis_data}"
                )
    ...
    async def _build_enhanced_context(
        self, interaction_state, user_profile, relevant_memories, crisis_assessment
    ):
    ...
    async def _generate_enhanced_companion_response(
        self,
        user_input: str,
        analysis_context,
        guidance_package,
        ...
        base_response = self.mood_inflection.apply_inflection(
            base_response, guidance_package.primary_mode, mood
        )
    ...
    async def _generate_enhanced_fallback_response(
        self, user_input: str, error_msg: str, user_id: str
    ):
        ...
        if user_id in self.interaction_states:
            ...
            if interaction_state.interaction_count > 0:
                user_context = (
                    "I know we've been working together, and that connection is important to me. "
                )
