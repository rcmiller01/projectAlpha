...
class N8NWorkflowManager:
        ...
        self.persona_policies = {
            ...
            "lyra": {
                ...
                "approval_required_for": [
                    "creative_workflow_changes",
                    "inspiration_time_alterations",
                ],
                "monitoring_preferences": {
    ...
    async def create_workflow(
        self, workflow_config: dict[str, Any], persona_name: str
    ) -> dict[str, Any]:
        """
    ...
    async def _assess_workflow_safety(
        self, workflow_config: dict[str, Any], persona_name: str
    ) -> dict[str, Any]:
        ...
        approved = (
            ...
            and risk_assessment["overall_risk"] <= self._get_persona_risk_threshold(persona_name)
            and persona_evaluation["aligns_with_values"]
        )
        ...
        return {
            ...
            "reason": self._generate_assessment_reason(
                approved, safety_violations, persona_evaluation
            ),
    ...
    def _evaluate_with_persona_values(
        ...
        overall_alignment = sum(value_alignment_scores.values()) / max(
            1, len(value_alignment_scores)
        )

        # Check if approval required
        approval_required_for = persona_policy.get("approval_required_for", [])
        requires_approval = any(
            requirement in workflow_config.get("data_types", [])
    ...
    async def _enhance_workflow_with_persona_oversight(
        self, workflow_config: dict[str, Any], persona_name: str, safety_assessment: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        if persona_policy["monitoring_preferences"].get("emotional_impact_tracking", False):
            enhanced_config["emotional_tracking"] = self._create_emotional_tracking_config(
                workflow_config
            )

        return enhanced_config

    def _create_persona_monitoring_nodes(
        self, persona_name: str, workflow_config: dict[str, Any]
    ) -> list[dict[str, Any]]:
    ...
    async def execute_workflow(
        ...
        try:
            ...
            execution.status = (
                WorkflowStatus.COMPLETED if n8n_result["success"] else WorkflowStatus.FAILED
            )
    ...
    async def _persona_pre_execution_check(
        self, workflow_info: dict[str, Any], trigger_data: dict[str, Any], persona_name: str
    ) -> dict[str, Any]:
        ...
        if persona_name == "mia":
            ...
            data_quality_issues = await self._check_data_quality_for_workflow(
                workflow_info, trigger_data
            )
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if response.status_code == 201:
                    ...
                    return {"success": True, "workflow_id": result.get("id"), "data": result}
                else:
                    return {
                        "success": False,
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if response.status_code == 200:
                    result = response.json()
                    return {"success": True, "execution_data": result}
                else:
                    return {
                        "success": False,
                        "error": f"HTTP {response.status_code}: {response.text}",
    ...
    def _generate_workflow_creation_guidance(
        self, workflow_config: dict[str, Any], persona_name: str
    ) -> str:
    ...
    def _get_monitoring_info(self, workflow_id: str) -> dict[str, Any]:
        ...
        workflow_info = self.active_workflows[workflow_id]
        return {
            "monitoring_level": workflow_info.get(
    ...
    def _check_safety_violations(
        self, workflow_config: dict[str, Any], workflow_type: str
    ) -> list[str]:
        ...
        for action in actions:
            ...
            if "preserve_personal_relationships" in safety_rules and action_type in [
                "email_send",
                "message_send",
            ]:
    ...
    def _generate_assessment_reason(
        self, approved: bool, safety_violations: list[str], persona_evaluation: dict[str, Any]
    ) -> str:
        ...
        if not persona_evaluation["aligns_with_values"]:
            reasons.append(
                f"Doesn't align with persona values (score: {persona_evaluation['overall_alignment']:.2f})"
            )

        return "; ".join(reasons) if reasons else "Assessment failed for unknown reasons"

    def _generate_persona_feedback(
        self, persona_evaluation: dict[str, Any], persona_name: str
    ) -> str:
    ...
    def _generate_safety_suggestions(
        self, safety_violations: list[str], risk_assessment: dict[str, Any]
    ) -> list[str]:
    ...
    def _generate_persona_recommendation(
        self, alignment_score: float, automation_philosophy: str
    ) -> str:
    ...
    def _create_safety_checkpoints(
        self, safety_assessment: dict[str, Any], persona_policy: dict[str, Any]
    ) -> list[dict[str, Any]]:
    ...
    def _create_persona_guidance_points(
        self, workflow_config: dict[str, Any], persona_name: str
    ) -> list[dict[str, Any]]:
    ...
    def _create_rollback_mechanisms(
        self, workflow_config: dict[str, Any], persona_policy: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _create_emotional_tracking_config(self, workflow_config: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "alert_thresholds": {"stress_increase": 0.2, "satisfaction_decrease": 0.3},
        }

    async def _monitor_execution_with_persona(
        self, execution: WorkflowExecution, n8n_result: dict[str, Any], persona_name: str
    ) -> dict[str, Any]:
    ...
    async def _persona_post_execution_analysis(
        self, execution: WorkflowExecution, workflow_info: dict[str, Any], persona_name: str
    ) -> dict[str, Any]:
    ...
    def _workflow_aligns_with_current_goals(
        self, workflow_info: dict[str, Any], context: dict[str, Any]
    ) -> bool:
    ...
    async def _check_data_quality_for_workflow(
        self, workflow_info: dict[str, Any], trigger_data: dict[str, Any]
    ) -> list[str]:
    ...
    def _analyze_workflow_performance(self, n8n_result: dict[str, Any]) -> dict[str, float]:
        ...
        return {
            ...
            "efficiency_score": min(
                1.0, 1.0 - (execution_time / 60000)
            ),  # Normalize to 1-minute baseline
        }
    ...
    def _save_workflow_data(self):
        ...
        try:
            ...
            data = {
                ...
                "workflow_executions": [
                    asdict(execution) for execution in self.workflow_executions[-100:]
                ],
                ...
                if "active_workflows" in data:
                    for workflow_id, workflow_data in data["active_workflows"].items():
                        ...
                        if workflow_data["last_execution"]:
                            workflow_data["last_execution"] = datetime.fromisoformat(
...
async def create_persona_supervised_workflow(
    n8n_base_url: str,
    api_key: str,
    user_id: str,
    workflow_config: dict[str, Any],
