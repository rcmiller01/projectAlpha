...
class InvestmentTracker:
    ...
    def _calculate_max_gain_loss(
        self, strategy_type: StrategyType, legs: list[OptionsLeg]
    ) -> tuple[float, float]:
    ...
    def _assess_risk_level(
        self, strategy_type: StrategyType, max_loss: float, entry_cost: float
    ) -> RiskLevel:
    ...
    def _generate_plain_english_review(
        self,
        ticker: str,
        strategy_type: StrategyType,
        legs: list[OptionsLeg],
        ...
        if template:
            review_parts.append(
                f"This {strategy_type.value.replace('_', ' ')} strategy on {ticker} involves {template['description'].lower()}."
            )

        # Financial metrics
        if max_gain != float("inf") and max_loss != float("inf"):
            review_parts.append(
                f"Your maximum potential gain is ${max_gain:.2f} and maximum risk is ${max_loss:.2f}."
            )
        ...
        if template:
            review_parts.append(
                f"This strategy works best when {template['best_conditions'].lower()}."
            )
    ...
    def _generate_recommendation(
        ...
        if risk_level in [RiskLevel.VERY_LOW, RiskLevel.LOW]:
            recommendations.append(
                "This is a relatively safe strategy that aligns well with conservative risk management."
            )
        elif risk_level == RiskLevel.MODERATE:
            recommendations.append(
                "This strategy has moderate risk - appropriate if you're comfortable with the potential loss amount."
            )
        else:
            recommendations.append(
                "This is a higher-risk strategy. Only consider if you can afford the maximum loss and have experience."
            )
        ...
        if user_context:
            ...
            if experience_level == "beginner" and risk_level != RiskLevel.LOW:
                recommendations.append(
                    "As you're building experience, consider starting with lower-risk strategies."
                )

            if risk_tolerance == "conservative" and risk_level in [
                RiskLevel.HIGH,
                RiskLevel.VERY_HIGH,
            ]:
                recommendations.append(
                    "This strategy may not align with your conservative risk tolerance."
                )

        return " ".join(recommendations)

    def _generate_emotional_context(
        self,
        risk_level: RiskLevel,
        max_gain: float,
        max_loss: float,
        ...
        if risk_level in [RiskLevel.VERY_LOW, RiskLevel.LOW]:
            emotional_phrases.append(
                "This feels like a comfortable play that should let you sleep well at night."
            )
        elif risk_level == RiskLevel.MODERATE:
            emotional_phrases.append(
                "There's a balance of opportunity and risk here that could work well for building confidence."
            )
        else:
            emotional_phrases.append(
                "This is more aggressive - make sure you're emotionally prepared for the potential outcomes."
            )
        ...
        if max_gain != float("inf") and max_loss != float("inf"):
            ...
            if ratio > 0.5:
                ...
                emotional_phrases.append(
                    "The risk is higher than the reward, so this is more about probability than big gains."
                )

        # Encouraging context
        if user_context and user_context.get("building_confidence"):
            emotional_phrases.append(
                "This could be a good learning experience to build your options trading confidence."
            )
    ...
    def log_trade_result(
        ...
        profit_percentage = (
            (profit_loss / abs(strategy.entry_cost)) * 100 if strategy.entry_cost != 0 else 0
        )
    ...
    def _generate_emotional_impact(
        self, profit_loss: float, profit_percentage: float, outcome: str
    ) -> str:
    ...
    def get_performance_summary(self, days: int = 30) -> dict[str, Any]:
        ...
        recent_trades = [trade for trade in self.trade_history if trade.exit_date >= cutoff_date]
        ...
        return {
            ...
            "largest_win": max(
                (t.profit_loss for t in recent_trades if t.outcome == "win"), default=0
            ),
            "largest_loss": min(
                (t.profit_loss for t in recent_trades if t.outcome == "loss"), default=0
            ),
        }
