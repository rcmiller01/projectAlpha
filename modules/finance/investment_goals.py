...
class InvestmentGoalsTracker:
    ...
    def add_contribution(
        ...
        logger.info(
            f"Added ${amount:.2f} to goal '{goal.name}' - now at {goal.progress_percentage:.1f}%"
        )
        return result
                ...
                suggestions.append(
                    {
                        ...
                        "new_progress": min(
                            100.0,
                            ((goal.current_amount + suggested_amount) / goal.target_amount) * 100,
                        ),
    ...
    def get_goals_summary(self, include_completed: bool = False) -> dict[str, Any]:
        ...
        attention_goals = [
            ...
            if goal.status == GoalStatus.ACTIVE
            and goal.progress_percentage < 25
            and goal.priority <= 2
        ]
    ...
    def _check_milestone_achievement(
        self, goal: InvestmentGoal, previous_amount: float
    ) -> Optional[str]:
        """Check if a milestone was achieved and return celebration message"""
        previous_percentage = (
            (previous_amount / goal.target_amount * 100) if goal.target_amount > 0 else 0
        )
    ...
    def _generate_completion_message(self, goal: InvestmentGoal) -> str:
        ...
        base_message = (
            f"🎉 GOAL COMPLETED: '{goal.name}' is fully funded at ${goal.current_amount:.2f}!"
        )
    ...
    def _generate_encouragement_message(
        self, goal: InvestmentGoal, contribution_amount: float
    ) -> str:
    ...
    def _generate_allocation_encouragement(self, suggestions: list[dict[str, Any]]) -> str:
        ...
        if high_impact:
            ...
            if len(goal_names) == 1:
                return (
                    f"This profit could complete your {goal_names[0]} goal! That would be amazing!"
                )
            else:
                return f"This profit could complete {' and '.join(goal_names)}! What a great trading session!"
        else:
            return (
                "Your trading profits are building toward your goals - every contribution adds up!"
            )

    def _generate_default_emotional_value(self, goal_type: GoalType, name: str) -> str:
        """Generate default emotional value for goal type"""
        emotional_defaults = {
            GoalType.VACATION: "Taking time to recharge and create memories",
            GoalType.TECH_UPGRADE: "Improving our digital life and capabilities",
            GoalType.LEARNING_FUND: "Investing in knowledge and personal growth",
            GoalType.EMERGENCY_FUND: "Building security and peace of mind",
            GoalType.HOBBY_FUND: "Supporting creativity and personal interests",
            GoalType.GIFT_FUND: "Showing care for people who matter",
            GoalType.EXPERIENCE: "Creating meaningful memories and experiences",
        }
...
if __name__ == "__main__":
    ...
    print("Vacation Goal Contribution:")
    ...
    for suggestion in suggestions["suggestions"]:
        ...
        print(
            f"    Current: {suggestion['current_progress']:.1f}% → New: {suggestion['new_progress']:.1f}%"
        )
        print(f"    {suggestion['emotional_impact']}")
        if suggestion["would_complete"]:
            print("    🎉 This would COMPLETE the goal!")
        print()
    ...
    for goal in summary["goals"]:
        print(
            f"  - {goal['name']}: {goal['progress_percentage']:.1f}% (${goal['current_amount']:.2f}/${goal['target_amount']:.2f})"
        )
