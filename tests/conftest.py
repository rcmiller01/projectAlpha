...
def mock_memory_system():
    ...
    with patch('core.memory_system.MemorySystem') as mock:
        ...
        # Mock memory layers
        memory_instance.long_term_memory = {
            'identity': [],
            'beliefs': [],
            'ephemeral': []
        }
        ...
        memory_instance.get_memory_quota_status.return_value = {
            'identity': {'current_items': 0, 'max_items': 10, 'usage_percentage': 0.0, 'is_over_quota': False},
            'beliefs': {'current_items': 0, 'max_items': 50, 'usage_percentage': 0.0, 'is_over_quota': False},
            'ephemeral': {'current_items': 0, 'max_items': 100, 'usage_percentage': 0.0, 'is_over_quota': False}
        }
        
        memory_instance.add_layered_memory.return_value = True
        memory_instance.prune_all_layers.return_value = {'identity': 0, 'beliefs': 0, 'ephemeral': 0}
    ...
    class DriftChecker:
        ...
        def _check_monotonic_sections(self) -> bool:
            ...
            for i in range(len(self.drift_values) - 2):
                ...
                if (all(a <= b for a, b in zip(section, section[1:])) or
                    all(a >= b for a, b in zip(section, section[1:]))):
                    return True
            return False
...
pytest.mark.deterministic = pytest.mark.skipif(
    not os.environ.get("PYTEST_DETERMINISTIC"),
    reason="Requires --deterministic flag"
)

pytest.mark.chaos = pytest.mark.skipif(
    os.environ.get("PYTEST_NO_CHAOS") == "true",
    reason="Chaos tests disabled"
)

pytest.mark.slow = pytest.mark.skipif(
    os.environ.get("PYTEST_FAST") == "true",
    reason="Slow tests skipped in fast mode"
)
