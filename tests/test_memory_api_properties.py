...
from backend.common.security import can_access_layer, get_token_type, mask_token
...
class TestMemoryAPIInvariants:
    ...
    @given(
        ...
        token=st.sampled_from(
            ["admin_test_token", "sys_test_token", "user_test_token", "invalid_token"]
        ),
    )
        ...
        try:
            ...
            if layer == "identity":
                ...
                if token_type != "admin":
                    ...
                    assert (
                        not can_access
                    ), f"Non-admin token {mask_token(token)} should not access identity layer"
                else:
                    # Admin tokens should be able to access identity layer
                    assert can_access, "Admin token should access identity layer"
    ...
    def test_rate_limit_eventually_blocks_spammy_sources(
        self, source_ip, request_count, time_window, rate_limit_tracker
    ):
        """
        ...
        if request_count > RATE_LIMIT_MAX:
            ...
            assert (
                blocked_count >= expected_blocked * 0.8
...
class MemoryStateMachine(RuleBasedStateMachine):
    ...
    def setup(self):
        ...
        # Ensure clean state
        self.memory_system.long_term_memory = {"identity": [], "beliefs": [], "ephemeral": []}
        self.operation_count = 0
    ...
    def add_memory(self, layer, content, importance):
        ...
        if success:
            self.added_memories[layer].append(
                {"content": content, "importance": importance, "operation": self.operation_count}
            )
    ...
    def check_memory_exists(self, memories):
        ...
        if was_added:
            ...
            memory_exists = any(
                mem.get("content", "").startswith(content[:20]) for mem in layer_memories
            )
    ...
    def check_quota_status(self):
        ...
        for layer, layer_status in status.items():
            if layer in ["identity", "beliefs", "ephemeral"]:
                ...
                assert (
                    layer_status["current_items"] <= layer_status["max_items"] + 1
                ...
                assert (
                    0.0 <= layer_status["usage_percentage"] <= 105.0
    ...
    def test_idempotency_key_consistency(self, idempotency_key, request_data, repeat_count):
        ...
        for i in range(repeat_count):
            ...
            if cached_response is not None and "data" in cached_response:
                assert (
                    cached_response["data"] == request_data
...
class TestDeterministicBehavior:
    ...
    def test_deterministic_drift_calculation(self, deterministic_mode):
        ...
        np.testing.assert_array_equal(
            drift_values1, drift_values2, "Drift calculations should be deterministic"
        )
