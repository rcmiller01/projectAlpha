...
class DriftJournalTester:
            ...
            if response.status_code == 200:
                ...
                if data.get("status") == "healthy":
                    self.log_test(
                        "API Health Check",
                        True,
                        f"Server healthy with {data.get('data_status', {}).get('drift_entries', 0)} entries",
                    )
                    return True
                else:
    ...
    def test_drift_history_endpoint(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if len(entries) > 0:
                    self.log_test(
                        "Drift History Default", True, f"Retrieved {len(entries)} entries"
                    )
                    ...
                    if not missing_fields:
                        self.log_test(
                            "Entry Structure Validation", True, "All required fields present"
                        )
                    else:
                        self.log_test(
                            "Entry Structure Validation", False, f"Missing fields: {missing_fields}"
                        )
                else:
            ...
            for time_range in ["day", "week", "month"]:
                response = requests.get(
                    f"{self.api_url}/api/drift/history?range={time_range}", timeout=5
                )
                if response.status_code == 200:
                    data = response.json()
                    entries = data.get("entries", [])
                    self.log_test(
                        f"Time Range Filter ({time_range})", True, f"{len(entries)} entries"
                    )
                else:
                    self.log_test(
                        f"Time Range Filter ({time_range})", False, f"HTTP {response.status_code}"
                    )
            ...
            if response.status_code == 200:
                ...
                if not missing_fields:
                    ...
                    if isinstance(total_drifts, int) and total_drifts >= 0:
                        ...
                        self.log_test(
                            "Total Drifts Validation", False, "Invalid total_drifts value"
                        )
                    ...
                    if isinstance(avg_magnitude, (int, float)) and 0 <= avg_magnitude <= 1:
                        ...
                        self.log_test(
                            "Average Magnitude Validation", False, "Invalid average_magnitude value"
                        )

                    if isinstance(timeline_data, list) and len(timeline_data) > 0:
                        self.log_test(
                            "Timeline Data Validation", True, f"{len(timeline_data)} data points"
                        )
                    else:
                    ...
                    self.log_test(
                        "Drift Summary Structure", False, f"Missing fields: {missing_fields}"
                    )
            else:
    ...
    def test_drift_actions(self):
        ...
        try:
            ...
            response = requests.post(
                f"{self.api_url}/api/drift/approve", json=approval_data, timeout=5
            )
            if response.status_code == 200:
            ...
            response = requests.post(
                f"{self.api_url}/api/drift/annotate", json=annotation_data, timeout=5
            )
            if response.status_code == 200:
            ...
            if len(entries) > 1:
                ...
                response = requests.post(
                    f"{self.api_url}/api/drift/revert", json=revert_data, timeout=5
                )
                if response.status_code == 200:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "auto_generate" in config and "drift_sensitivity" in config:
                    ...
                    update_data = {"drift_sensitivity": 0.7, "max_history_entries": 75}
                    response = requests.post(
                        f"{self.api_url}/api/drift/config", json=update_data, timeout=5
                    )
                    if response.status_code == 200:
            ...
            if os.path.exists(filename):
                try:
                    ...
                    self.log_test(
                        ...
                        True,
                        f"Valid JSON with {len(data) if isinstance(data, (list, dict)) else 'N/A'} items",
                    )
                except json.JSONDecodeError:
                    self.log_test(f"Data File: {filename}", False, "Invalid JSON format")
                except Exception as e:
                    self.log_test(f"Data File: {filename}", False, f"Read error: {e!s}")
            else:
        ...
        if os.path.exists(component_file):
            try:
                ...
                if not missing_features:
                    ...
                    self.log_test(
                        "Component Structure", False, f"Missing features: {missing_features}"
                    )
                ...
                self.log_test("Component File Reading", False, f"Error: {e!s}")
        else:
    ...
    def generate_test_report(self):
        ...
        api_success = (
            len([r for r in api_tests if r["success"]]) / len(api_tests) if api_tests else 0
        )

        component_tests = [
            r for r in self.test_results if "Component" in r["test"] or "React" in r["test"]
        ]
        component_success = (
            len([r for r in component_tests if r["success"]]) / len(component_tests)
            if component_tests
            else 0
        )
        ...
        data_success = (
            len([r for r in data_tests if r["success"]]) / len(data_tests) if data_tests else 0
        )
