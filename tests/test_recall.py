...
class TestTrueRecallSystem:
    ...
    async def memory_store(self, temp_storage):
        ...
        store = MemoryStore(
            temp_storage, {"use_sqlite": True, "use_jsonl": True, "auto_backup": False}
        )
...
class TestMemoryGraph:
    ...
    async def test_time_range_filtering(self, memory_graph, sample_events):
        ...
        filtered_events = await memory_graph.search_events(time_range=(start_time, end_time))
...
class TestReflectionAgent:
    ...
    async def test_weekly_reflection_generation(
        self, reflection_agent, memory_graph, sample_events
    ):
...
class TestTrueRecallEngine:
    ...
    async def recall_engine(self, temp_storage):
        ...
        config = {"storage_path": temp_storage, "use_sqlite": True, "use_jsonl": True}
        ...
        try:
            ...
            for i, (actor, event_type, content, expected_tone) in enumerate(events_to_record):
                ...
                await engine.record_event(
                    actor=actor, event_type=event_type, content=content, timestamp=timestamp
                )
...
if __name__ == "__main__":
    ...
    print(
        "📊 All components tested: Emotion Analysis, Salience Scoring, Memory Graph, Reflection Agent, Storage, Integration"
    )
