...
def check_rate_limit(session_token: str) -> bool:
    ...
    # Clean old requests
    while (
        rate_limit_requests[session_token]
        and rate_limit_requests[session_token][0] < current_time - RATE_LIMIT_WINDOW
    ):
...
def validate_drift_entry(entry_data: dict[str, Any]) -> tuple[bool, str]:
    ...
    try:
        ...
        logger.error(f"Error validating drift entry: {e!s}")
...
def detect_drift_anomaly(entry_data: dict[str, Any]) -> bool:
    ...
    try:
        ...
        if recent_count > 5:
            logger.warning(
                f"Rapid drift succession anomaly detected: {recent_count} drifts in 5 minutes"
            )
            drift_anomalies.append(
                {
                    "timestamp": datetime.now().isoformat(),
        ...
        logger.error(f"Error detecting drift anomaly: {e!s}")
        return False
...
def log_drift_access(
    endpoint: Optional[str], session_token: Optional[str], action: str, status: str = "success"
):
    ...
    try:
        ...
        if status != "success":
            logger.warning(
                f"Drift access issue: {endpoint or 'unknown'} - {action} failed with {status}"
            )

    except Exception as e:
        logger.error(f"Error logging drift access: {e!s}")
...
def generate_realistic_drift_entry():
    ...
    ritual_contexts = [
        ...
        "Trust building ritual",
        "Vulnerability ceremony",
    ]
...
def get_drift_summary():
    ...
    try:
        ...
        for cause in drift_types:
            drift_types[cause]["intensity"] = (
                drift_types[cause]["total_intensity"] / drift_types[cause]["count"]
            )
...
def annotate_drift():
    ...
    try:
        ...
        # Save both files
        save_success = save_json_file(DRIFT_ANNOTATIONS_FILE, drift_annotations) and save_json_file(
            DRIFT_HISTORY_FILE, drift_history
        )
...
def generate_drift_entry():
    ...
    try:
        ...
        if save_json_file(DRIFT_HISTORY_FILE, drift_history):
            return jsonify(
                {"message": "New drift entry generated", "entry": new_entry, "status": "success"}
            )
        else:
    ...
    return jsonify(
        {
            ...
            "data_status": {
                ...
                "pending_actions": sum(
                    1 for entry in drift_history if entry.get("requires_action")
                ),
            },
        }
    )
