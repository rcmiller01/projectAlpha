...
def generate_ritual_data():
    ...
    for template in symbol_templates:
        ...
        symbol["salience_score"] = max(
            0.1, min(1.0, template["base_salience"] + salience_variation)
        )
...
def get_active_rituals():
    ...
    try:
        ...
        all_rituals = sorted(
            active_rituals, key=lambda x: (-x.get("frequency", 0), x.get("name", ""))
        )
    ...
    try:
        ...
        for i in range(random.randint(5, 12)):
            ...
            entry = {
                ...
                "ritual_connection": random.choice(
                    symbol.get("ritual_connections", ["general_practice"])
                ),
            }
...
def update_symbol():
    ...
    try:
        ...
        return jsonify(
            {"message": "Symbol updated successfully", "symbol": symbol, "status": "success"}
        )
    ...
    return jsonify(
        {
            ...
            "data_status": {
                ...
                "available_rituals": len(
                    [r for r in active_rituals if r.get("is_available", True)]
                ),
