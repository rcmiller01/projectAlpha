...
def cleanup_idempotency_cache():
    ...
    expired_keys = [
        key
        for key, (_, timestamp) in idempotency_cache.items()
...
@validate_json_schema(
    {
        ...
        "required": ["strategy"],
        "properties": {
            "strategy": {"type": "string", "enum": ["balanced", "emotional", "logical", "creative"]}
        },
    }
)
    ...
    try:
        ...
        if emotional_state_path.exists():
            ...
            state = {
                ...
                "stability": 0.85,
                "mood_signals": {"warmth": 0.7, "empathy": 0.8, "curiosity": 0.6, "concern": 0.3},
            }
...
def get_traces() -> Response:
    ...
    try:
        ...
        if trace_path.exists():
            ...
            return jsonify({"traces": traces[-limit:], "total_count": len(traces)})
        else:
            return jsonify({"traces": [], "total_count": 0})
    ...
    try:
        ...
        try:
            ...
            response_mood = {
                ...
                "icon": (
                    "🤔"
                    if response.tone == "objective"
                    else "💭"
                    if response.tone == "balanced"
                    else "💖"
                ),
            }
...
def health_check() -> Response:
    ...
    try:
        ...
        return (
            jsonify({"status": "error", "error": str(e), "timestamp": datetime.now().isoformat()}),
            500,
        )
