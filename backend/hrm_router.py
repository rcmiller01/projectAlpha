...
def cleanup_hrm_cache():
    ...
    expired_keys = [
        key
        for key, (_, timestamp) in hrm_idempotency_cache.items()
...
class HRMRouter:
    ...
    def route_request(
        ...
        try:
            ...
            log_audit_entry(audit_entry)
            return {"error": f"Internal error: {e!s}", "request_id": request_id}

    def _handle_identity_request(
        self, operation: str, data: Optional[dict[str, Any]], token: Optional[str], request_id: str
    ) -> dict[str, Any]:
    ...
    def _handle_beliefs_request(
        ...
        if token_type not in ["admin", "system"]:
            return {
                "error": "Admin or system access required for beliefs layer",
                "request_id": request_id,
            }
    ...
    def _handle_ephemeral_request(
        ...
        if not token_type:
            return {
                "error": "Authentication required for ephemeral layer",
                "request_id": request_id,
            }
